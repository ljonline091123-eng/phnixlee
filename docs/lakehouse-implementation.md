...
- `POST /api/v1/lakehouse/quality/assess` 在发布前执行确定性质量预检，检查必填字段、业务键重复、非法数值、行情高低价区间、正文可用性和来源追溯。
- Raw 层以原始留存为优先，问题记录可以归档但必须产生警告；Normalized 和 Serving 层的关键契约失败时拒绝发布。
- 数据质量契约版本保存在每个数据集版本的 `quality_json` 中，发布接口会重新执行门禁，不能只依赖前端检查。
- 文档切片默认使用 `STRUCTURE_V1`：优先在段落、换行、句末和子句边界切分，保存原文偏移、章节标题、边界类型、解析器版本和正文质量结果。历史 `TEXT_V1` 切片继续保留。
- 采集、去重、质量门禁、切片和分层发布保持 Python/SQL 确定性执行；智能体和 Skill 只用于来源审阅、异常解释和语义候选，不得绕过门禁或直接发布已验证事实。
