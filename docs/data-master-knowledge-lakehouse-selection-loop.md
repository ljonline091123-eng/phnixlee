...
本轮使用隔离数据库完成 11 只 A 股、6 只港股、1 只新三板基础层和 2 只创新层证券的验收，未修改主业务库。范围为：

- A 股：000001、000002、000006、000008、000011、000682、000716、002533、600519、688627、689009；
- 港股：00001、00002、00003、00008、00700、02533；
- 新三板：430019；创新层：832327、875228。

20/20 均完成股票—公司身份解析、知识文档、图谱路径、有效切片、湖仓数据集和精确血缘校验，且 GraphRAG 上下文版本均为 `GRAPH_RAG_CONTEXT_V3`。四个市场投影分别形成：

| 范围 | 文档 | 实体 | 关系 |
| --- | ---: | ---: | ---: |
| A 股 11 只 | 1,718 | 2,459 | 4,919 |
| 港股 6 只 | 681 | 860 | 1,704 |
| 新三板 1 只 | 111 | 152 | 287 |
| 创新层 2 只 | 172 | 226 | 434 |

选股闭环另行验证了未治理图谱零模型调用、已治理图谱的结构化输出和引用白名单、人工复选、10 个有效交易日快照、不可变复盘、历史盲测及修复草案去重。所有模型步骤均使用进程内无网络桩；该结果只证明编排、状态门禁和审计契约有效，不代表真实 LLA 的选股质量。

最终自动化验收为后端 `421 passed, 305 subtests passed`，前端 TypeScript 与生产构建通过。当前 20 股投影仍为 `PENDING`，必须完成治理后才能调用真实模型；外部事件、经验证供应链和司法事实等真实数据缺口仍按第 7 节处理。
