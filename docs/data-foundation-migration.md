...
本轮新增独立的 Alembic 历史，当前版本为 `foundation_0001`。它只管理以下七张表及自己的版本表 `foundation_schema_revision`：
...
1. 尚未启动新版本：七张新表不存在，迁移创建这些表并记录版本。
...
`preview` 展示七张表与版本表的完整初始 DDL，不根据某个库的现状裁剪 SQL。实际待创建表和需核对采用的表由 `check` 给出。不要将完整预览 SQL 直接执行到已有表的数据库。
...
| `CURRENT` | 新表与冻结结构一致，版本为 `foundation_0001` | 重复 `apply` 不重复建表或写入版本 |
...
- 本次仅有首版历史，没有自动生成后续全仓库迁移的能力。后续结构变更应新增经过评审的版本及对应检查和测试，不修改已发布首版的结构快照，不把旧业务表交给自动差异脚本。

冻结快照位于 `backend/alembic/versions/foundation_0001.py`，不导入可变 ORM 模型。测试同时校验 ORM `create_all` 的结构与该快照一致，避免应用和迁移悄然分叉。
...
本轮没有连接用户业务数据库，没有执行真实库迁移。PostgreSQL 在线迁移尚未使用实际实例验证，投产前需在目标版本的独立测试库验证。
