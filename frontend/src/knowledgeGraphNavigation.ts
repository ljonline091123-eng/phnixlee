...
export function belongsToCategory(node: NetworkNode, category: GraphCategory, edges: NetworkEdge[]): boolean {
  ...
  switch (category) {
    ...
    case "disclosure": return documentTypes.has(type);
...
export function buildNavigationGroups(data: KnowledgeNetwork | null) {
  ...
  return graphCategories.map(category => {
    const nodes = (data?.nodes || []).filter(node => !core.has(node.id) && belongsToCategory(node, category.key, data?.edges || []));
...
export function buildDomainGraph(data: KnowledgeNetwork | null, category: GraphCategory, page = 0, subtype = "", pageSize = 8) {
  ...
  const all = data.nodes.filter(node => !coreIds.has(node.id) && belongsToCategory(node, category, data.edges));
  ...
  const pages = Math.ceil(filtered.length / pageSize), actualPage = Math.max(0, Math.min(page, Math.max(0, pages - 1)));
  ...
  data.edges.forEach(edge => { adjacency.set(edge.source, [...(adjacency.get(edge.source) || []), edge.target]); adjacency.set(edge.target, [...(adjacency.get(edge.target) || []), edge.source]); });
  ...
  return { nodes: data.nodes.filter(node => ids.has(node.id)), edges: data.edges.filter(edge => ids.has(edge.source) && ids.has(edge.target)), records, total: filtered.length, pages, page: actualPage, types, omittedContext };
}
