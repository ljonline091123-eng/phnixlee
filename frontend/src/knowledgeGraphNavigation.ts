...
// Evidence is attached to each object and relation in the inspector. Keeping it
// out of the guided first level avoids presenting provenance as business knowledge.
export const securityCategoryKeys: GraphCategory[] = ["identity", "classification", "ownership", "financial", "market", "disclosure"];
export const companyCategoryKeys: GraphCategory[] = ["identity", "classification", "ownership", "business", "legal", "disclosure"];
...
export function stableDomainLayout(nodes: NetworkNode[], centerId: string, coreIds: string[] = [], overview = false, expandedCategory?: string): Record<string, { x: number; y: number }> {
  const points: Record<string, { x: number; y: number }> = {};
  if (overview) {
    nodes.forEach((node, i) => { points[node.id] = { x: (i - (nodes.length - 1) / 2) * 280, y: 0 }; });
    return points;
  }
  const core = nodes.filter(node => coreIds.includes(node.id) || node.id === centerId);
  const categories = nodes.filter(node => node.type === "NAVIGATION_CATEGORY");
  const details = nodes.filter(node => !core.includes(node) && !categories.includes(node));
  core.forEach((node, i) => { points[node.id] = { x: (i - (core.length - 1) / 2) * 300, y: -285 }; });
  categories.forEach((node, i) => {
    const columns = Math.min(3, Math.max(1, categories.length));
    points[node.id] = { x: (i % columns - (columns - 1) / 2) * 275, y: -70 + Math.floor(i / columns) * 145 };
  });
  if (expandedCategory && details.length) {
    const categoryIndex = categories.findIndex(node => node.properties?.category === expandedCategory);
    const categoryPoint = categoryIndex >= 0 ? points[categories[categoryIndex].id] : { x: 0, y: 75 };
    const width = 220;
    const second = details.filter(node => node.properties?.ui_graph_level !== 3);
    const third = details.filter(node => node.properties?.ui_graph_level === 3);
    const place = (items: NetworkNode[], baseY: number) => {
      const columns = Math.min(4, Math.max(1, items.length));
      items.forEach((node, i) => { points[node.id] = { x: categoryPoint.x + (i % columns - (columns - 1) / 2) * width, y: baseY + Math.floor(i / columns) * 125 }; });
    };
    place(second, 245);
    place(third, 390 + Math.max(0, Math.ceil(second.length / 4) - 1) * 125);
  } else {
    const columns = details.length > 6 ? 3 : 2;
    details.forEach((node, i) => { points[node.id] = { x: (i % columns - (columns - 1) / 2) * 215, y: 90 + Math.floor(i / columns) * 130 }; });
  }
  return points;
}
