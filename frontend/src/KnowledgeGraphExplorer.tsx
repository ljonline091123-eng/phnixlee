...
import { ArrowLeft, ChevronLeft, ChevronRight, Download, Expand, Network, RefreshCw, Search, X, ZoomIn, ZoomOut } from "lucide-react";
...
import { buildDomainGraph, buildNavigationGroups, graphCategories, identityGraph, stableDomainLayout, type GraphCategory } from "./knowledgeGraphNavigation";
...
const typeNames: Record<string, string> = {
  SECURITY: "证券", STOCK: "证券", COMPANY: "公司", LEGAL_ENTITY: "公司", PERSON: "自然人", HOLDER_ACCOUNT: "股东账户",
  INDUSTRY: "行业", SECTOR: "板块", THEME: "主题", CLASSIFICATION: "类型标签", CLASSIFICATION_TAG: "类型标签", MARKET: "市场", EXCHANGE: "交易所",
  PRODUCT: "产品", MATERIAL: "原材料", SUPPLY_CHAIN_FACT: "供应链事实", OWNERSHIP_FACT: "股权事实", BUSINESS_FACT: "经营往来", LEGAL_CASE: "司法事项", LEGAL_FACT: "司法事项",
  ...
  EVIDENCE: "证据文档", EVIDENCE_DOCUMENT: "证据文档", DOCUMENT: "证据文档", SOURCE_DOCUMENT: "原始文档", NEWS: "新闻", NOTICE: "公告", RESEARCH_REPORT: "研究报告", F10_SECTION: "F10资料", DATA_SOURCE: "数据源", DATASET: "数据集", COMPANY_PROFILE: "公司资料",
};
const relationNames: Record<string, string> = {
  ...
  HAS_FINANCIAL_OBSERVATION: "财务观测", HAS_FINANCIAL_METRIC: "财务指标", HAS_FINANCIAL_REPORT: "财务报告", HAS_MARKET_OBSERVATION: "量价观测", HAS_MARKET_EVENT: "量价事件", HAS_MARKET_ANOMALY: "量价异常", HAS_PRICE_AND_VOLUME_SERIES: "历史量价", HAS_REALTIME_QUOTE: "实时行情", HAS_SHAREHOLDER_ACTION: "股东行为", HAS_SHAREHOLDER_EVENT: "股东事件", HAS_NEWS: "关联新闻", HAS_NOTICE: "关联公告", HAS_RESEARCH_REPORT: "研究报告", HAS_F10_SECTION: "F10资料", HAS_POLICY_EVENT: "政策事件", HAS_EXTERNAL_EVENT: "外部事件",
};
...
const nodeName = (node: NetworkNode) => node.type_label || typeNames[node.type] || (node.layer === "FACT" ? "事实记录" : node.layer === "EVIDENCE" ? "来源证据" : "关联对象");
...
export function KnowledgeGraphExplorer({ initialStock, initialCompanyId, initialGraphId, compact = false }: ExplorerProps) {
  ...
  const [navigation, setNavigation] = useState<"overview" | "domain" | "advanced">("overview");
  ...
  const svgRef = useRef<SVGSVGElement>(null), canvasRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    setCenter(next); setHistory([]); setHiddenTypes([]); setSearchOpen(!next); setNavigation("overview"); setCategory(null); setCategoryPage(0); setCategoryType("");
  ...
  const domain = useMemo(() => buildDomainGraph(data, category || "identity", categoryPage, categoryType, 8), [data, category, categoryPage, categoryType]);
  ...
  const visible = useMemo(() => {
    ...
    if (navigation === "domain") return { nodes: domain.nodes, edges: domain.edges };
  ...
  }, [data, layers, hiddenTypes, navigation, identity, domain]);
  const arranged = useMemo(() => navigation === "overview" ? stableDomainLayout(visible.nodes, data?.center_id || "", identity.nodes.map(node => node.id), true) : navigation === "domain" ? stableDomainLayout(visible.nodes, data?.center_id || "", identity.nodes.map(node => node.id)) : layout(visible.nodes, visible.edges, data?.center_id || ""), [visible, data?.center_id, navigation, identity.nodes]);
  ...
  function navigate(next: NetworkQuery) { autoSelect.current = false; if (center) setHistory(old => [...old.slice(-14), center]); setCenter(next); setHiddenTypes([]); setSearchOpen(false); setNavigation("overview"); setCategory(null); setCategoryPage(0); setCategoryType(""); }
  function openOverview() { setNavigation("overview"); setCategory(null); setCategoryPage(0); setCategoryType(""); setSelection(null); }
  function openCategory(key: GraphCategory) { setCategory(key); setCategoryPage(0); setCategoryType(""); setSelection(null); setNavigation("domain"); }
  ...
  function pointerUp(event: ReactPointerEvent<SVGSVGElement>) {
    const drag = pointer.current; if (drag?.id && !drag.moved) setSelection({ kind: "node", id: drag.id });
  ...
  return <section className={`knowledge-explorer kg-view-${navigation}${compact ? " kg-compact" : ""}`} aria-label="知识图谱可视化">
    ...
    <div className="kg-controls"><div className="kg-focus"><button type="button" title="返回上一个中心" aria-label="返回上一个中心" disabled={!history.length} onClick={() => { setCenter(history[history.length - 1]); setHistory(old => old.slice(0, -1)); }}><ArrowLeft size={15} /></button><strong>{loading ? "正在构建当前子图…" : currentNode?.label || "请选择股票或公司"}</strong>{data && <span>{visible.nodes.length} 个对象 · {visible.edges.length} 条关系</span>}</div><div className="kg-query-controls"><button type="button" className="kg-secondary-nav" data-testid="graph-overview" aria-label="返回图谱总览" onClick={() => { setNavigation("overview"); setCategory(null); setCategoryPage(0); setCategoryType(""); }}>总览</button><button type="button" className="kg-secondary-nav" data-testid="graph-advanced" aria-label="打开原始关系视图" onClick={() => setNavigation("advanced")}>原始关系</button><button type="button" aria-label="刷新图谱" disabled={!center || loading} onClick={() => setRevision(old => old + 1)}><RefreshCw size={15} /></button></div></div>
    {navigation === "advanced" && <div className="kg-filters"><span>高级设置 · 显示图层</span>{(Object.keys(layerNames) as NetworkLayer[]).map(layer => <label key={layer}><input type="checkbox" checked={layers.includes(layer)} onChange={() => setLayers(old => old.includes(layer) ? old.filter(item => item !== layer) : [...old, layer])} />{layerNames[layer]}</label>)}<label className="kg-candidate-toggle"><input type="checkbox" checked={candidates} onChange={event => setCandidates(event.target.checked)} />包含候选关系</label><label><input type="checkbox" checked={labels} onChange={event => setLabels(event.target.checked)} />关系名称</label><label>关联深度<select aria-label="关联深度" value={depth} onChange={e => setDepth(Number(e.target.value))}><option value={1}>1 层</option><option value={2}>2 层</option><option value={3}>3 层</option></select></label><label>对象上限<select aria-label="对象上限" value={nodeLimit} onChange={e => setNodeLimit(Number(e.target.value))}><option value={60}>60</option><option value={120}>120</option><option value={200}>200</option></select></label><label>每类资料<select aria-label="每类资料上限" value={perCategory} onChange={e => setPerCategory(Number(e.target.value))}><option value={3}>3 条</option><option value={5}>5 条</option><option value={10}>10 条</option></select></label><span className="kg-candidate-key">虚线表示待核实</span></div>}
    {navigation === "overview" && <section className="kg-domain-overview" aria-label="知识图谱领域导航"><div className="kg-domain-header"><div><strong>按领域查看</strong><small>概览仅保留证券与发行公司，其他资料按领域分组加载。</small></div><span>{groups.filter(group => group.loadedCount || group.sourceCount).length} 个领域有资料</span></div><div className="kg-domain-grid">{groups.map(group => <button type="button" key={group.key} data-domain={group.key} data-testid={`graph-domain-${group.key}`} aria-label={`查看${group.label}`} className="kg-domain-card" onClick={() => { setCategory(group.key); setCategoryPage(0); setCategoryType(""); setNavigation("domain"); }}><i style={{ background: group.tone }} /><strong>{group.label}</strong><small>{group.description}</small><span><b>{group.loadedCount}</b> 个已加载对象 · <b>{group.sourceCount}</b> 条来源记录</span></button>)}</div></section>}
    {navigation === "domain" && category && <section className="kg-domain-browser" aria-label="领域图谱浏览"><div className="kg-breadcrumb"><button type="button" data-testid="graph-back-overview" aria-label="返回图谱总览" onClick={() => { setNavigation("overview"); setCategory(null); }}>图谱总览</button><span>›</span><strong>{graphCategories.find(item => item.key === category)?.label}</strong><span className="kg-domain-count">{domain.total} 个对象 · 第 {domain.pages ? domain.page + 1 : 0}/{domain.pages || 0} 页</span></div><div className="kg-domain-toolbar"><label>对象类型<select aria-label="筛选对象类型" value={categoryType} onChange={event => { setCategoryType(event.target.value); setCategoryPage(0); }}><option value="">全部类型</option>{domain.types.map(type => <option value={type} key={type}>{typeNames[type] || type}</option>)}</select></label><div><button type="button" data-testid="graph-domain-prev" aria-label="上一页" disabled={domain.page <= 0} onClick={() => setCategoryPage(page => Math.max(0, page - 1))}><ChevronLeft size={14} /></button><button type="button" data-testid="graph-domain-next" aria-label="下一页" disabled={domain.page + 1 >= domain.pages} onClick={() => setCategoryPage(page => page + 1)}><ChevronRight size={14} /></button></div></div>{domain.omittedContext && <p className="kg-warning">为保持画布清晰，部分中间路径未展开；点击对象后可查看完整属性和证据。</p>}</section>}
    {data?.truncated && <p className="kg-warning" role="status">当前为有限子图，部分资料、对象或关系未展开。动态资料每类最多 {perCategory} 条，可提高展示上限，或双击证券、公司切换中心继续查看。</p>}
    ...
    <div className="kg-layout"><div className="kg-canvas-column"><div className="kg-canvas" ref={canvasRef}>
      <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" width={size.width} height={size.height} viewBox={`0 0 ${size.width} ${size.height}`} aria-label="股票公司关系画布" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => { pointer.current = null; }}>
        ...
        <g transform={`translate(${camera.x} ${camera.y}) scale(${camera.scale})`}>
          {visible.edges.map(edge => {
            ...
            const radius = edge.target === data?.center_id ? 35 : 25;
            ...
            return <g key={edge.id} className="kg-edge" role="button" tabIndex={0}
          ...
          {visible.nodes.map(node => { const point = positions[node.id]; if (!point) return null; const root = node.id === data?.center_id, selected = selection?.id === node.id, tone = color(node), r = root ? 33 : 23; return <g key={node.id} transform={`translate(${point.x} ${point.y})`} data-node-id={node.id} className="kg-node" role="button" tabIndex={0} aria-label={`${nodeName(node)}：${node.label}`} onDoubleClick={() => centerNode(node)} onKeyDown={event => { if (event.key === "Enter") { setSelection({ kind: "node", id: node.id }); } if (event.key === " ") { event.preventDefault(); centerNode(node); } }}><title>{node.label} · {nodeName(node)}{node.definition || node.properties?.definition ? `\n${node.definition || node.properties?.definition}` : ""}{isCenterable(node) ? "\n双击以此对象为中心" : ""}</title>{selected && <circle r={r + 7} fill="none" stroke={tone} strokeWidth="2" opacity=".4" />}{node.layer === "EVIDENCE" ? <rect x={-22} y={-22} width="44" height="44" rx="9" fill="#fff" stroke={tone} strokeWidth="2" strokeDasharray={isCandidate(node) ? "5 3" : undefined} /> : <circle r={r} fill={root ? tone : "#fff"} stroke={tone} strokeWidth={root ? 2 : 2.2} strokeDasharray={isCandidate(node) ? "5 3" : undefined} />}<text textAnchor="middle" dy="4" fontSize={root ? "12" : "10"} fill={root ? "#fff" : tone} fontFamily="sans-serif">{nodeName(node).slice(0, 4)}</text><text textAnchor="middle" y={r + 18} fill="#253d32" fontSize={root ? "12" : "11"} fontWeight={root ? "600" : "400"} fontFamily="sans-serif" paintOrder="stroke" stroke="#f7faf7" strokeWidth="4">{node.label.length > 13 ? `${node.label.slice(0, 12)}…` : node.label}</text></g>; })}
        </g>
      </svg>
      <div className="kg-canvas-tools"><button type="button" aria-label="放大图谱" onClick={() => zoom(1.3)}><ZoomIn size={17} /></button><button type="button" aria-label="缩小图谱" onClick={() => zoom(1 / 1.3)}><ZoomOut size={17} /></button><button type="button" aria-label="适配画布" onClick={() => setCamera(fitted(positions))}><Expand size={17} /></button><span>{Math.round(camera.scale * 100)}%</span></div>
