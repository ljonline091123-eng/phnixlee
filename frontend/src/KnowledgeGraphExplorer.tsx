...
import { buildDomainGraph, buildNavigationGroups, effectiveNodeType, graphCategories, identityGraph, stableDomainLayout, type GraphCategory } from "./knowledgeGraphNavigation";
...
export function KnowledgeGraphExplorer({ initialStock, initialCompanyId, initialGraphId, compact = false }: ExplorerProps) {
  ...
  return <section className={`knowledge-explorer kg-view-${navigation}${compact ? " kg-compact" : ""}`} aria-label="知识图谱可视化">
    ...
    <div className="kg-controls"><div className="kg-focus"><button type="button" title="返回上一个中心" aria-label="返回上一个中心" disabled={!history.length} onClick={() => { setCenter(history[history.length - 1]); setHistory(old => old.slice(0, -1)); openOverview(); }}><ArrowLeft size={15} /></button><strong>{loading ? "正在构建当前子图…" : currentNode?.label || "请选择股票或公司"}</strong>{data && <span>{visible.nodes.length} 个对象 · {visible.edges.length} 条关系</span>}</div><div className="kg-query-controls"><button type="button" className="kg-secondary-nav" data-testid="graph-overview" aria-label="返回图谱总览" onClick={openOverview}>总览</button><button type="button" className="kg-secondary-nav" data-testid="graph-advanced" aria-label="打开原始关系视图" onClick={() => { setNavigation("advanced"); setSelection(null); }}>原始关系</button><button type="button" aria-label="刷新图谱" disabled={!center || loading} onClick={() => setRevision(old => old + 1)}><RefreshCw size={15} /></button></div></div>
    ...
    {navigation === "domain" && category && <section className="kg-domain-browser" aria-label="领域图谱浏览"><nav className="kg-breadcrumb" aria-label="图谱层级"><button type="button" data-testid="graph-back-overview" aria-label="返回图谱总览" onClick={openOverview}>图谱总览</button><span>›</span><button type="button" className="kg-current-domain" aria-label="返回领域图谱" onClick={() => setSelection(null)}>{graphCategories.find(item => item.key === category)?.label}</button>{item && <><span>›</span><span className="kg-breadcrumb-record" title={item.label}>{selection?.kind === "edge" ? edgeName(item as NetworkEdge) : item.label}</span></>}<span className="kg-domain-count">已载入 {domain.total} 个对象 · 第 {domain.pages ? domain.page + 1 : 0}/{domain.pages || 0} 页</span></nav><div className="kg-domain-toolbar"><label>领域<select aria-label="切换图谱领域" value={category} onChange={event => openCategory(event.target.value as GraphCategory)}>{graphCategories.map(group => <option key={group.key} value={group.key}>{group.label}</option>)}</select></label><label>对象类型<select aria-label="筛选对象类型" value={categoryType} onChange={event => { setCategoryType(event.target.value); setCategoryPage(0); setSelection(null); }}><option value="">全部类型</option>{domain.types.map(type => <option value={type} key={type}>{typeNames[type] || "关联对象"}</option>)}</select></label><label className="kg-guided-candidate"><input type="checkbox" checked={candidates} onChange={event => setCandidates(event.target.checked)} />包含待核实线索</label><span className="kg-domain-pagination">每页 8 个对象<button type="button" data-testid="graph-domain-prev" aria-label="领域上一页" disabled={domain.page <= 0} onClick={() => { setCategoryPage(page => Math.max(0, page - 1)); setSelection(null); }}><ChevronLeft size={14} /></button><button type="button" data-testid="graph-domain-next" aria-label="领域下一页" disabled={domain.page + 1 >= domain.pages} onClick={() => { setCategoryPage(page => page + 1); setSelection(null); }}><ChevronRight size={14} /></button></span></div>{domain.omittedContext && <p className="kg-warning">为保持画布清晰，部分中间路径未展开；点击对象后可查看属性和证据。</p>}{!loading && !error && !domain.total && <p className="kg-domain-empty">{domainInfo?.sourceCount ? "此范围已有来源记录，但当前样本尚无可展开的这类对象。可调整类型筛选或读取更多资料。" : "当前查询范围尚无可展示资料，未采集不代表不存在相关事实。"}</p>}<div className="kg-domain-scope"><span>{domainInfo?.coverage.map(row => `${row.label} ${row.count.toLocaleString()}`).join(" · ") || "来源详情以对象及证据中的记录为准"}。图谱对象数与原始记录数口径不同。</span><button type="button" disabled={loading || perCategory >= 10 && nodeLimit >= 200} onClick={() => { setPerCategory(old => Math.min(10, old + 5)); setNodeLimit(200); }}>读取更多资料</button></div></section>}
      ...
      <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" width={size.width} height={size.height} viewBox={`0 0 ${size.width} ${size.height}`} aria-label="股票公司关系画布" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => { pointer.current = null; }}>
        ...
        <g transform={`translate(${camera.x} ${camera.y}) scale(${camera.scale})`}>
          ...
          {visible.nodes.map(node => {
            ...
            const root = node.id === data?.center_id, selected = selection?.id === node.id, tone = color(node), r = root ? 33 : 23;
            return <g key={node.id} transform={`translate(${point.x} ${point.y})`} data-node-id={node.id} className="kg-node" role="button" tabIndex={0} aria-label={`${nodeName(node)}：${node.label}`} opacity={selection && !neighbors.has(node.id) ? .28 : 1} onDoubleClick={() => centerNode(node)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelection({ kind: "node", id: node.id }); } }}>
              <title>{node.label} · {nodeName(node)}{node.definition || node.properties?.definition ? `\n${node.definition || node.properties?.definition}` : ""}{isCenterable(node) ? "\n双击以此对象为中心" : ""}</title>
              {navigation === "advanced" ? <>{selected && <circle r={r + 7} fill="none" stroke={tone} strokeWidth="2" opacity=".4" />}<circle r={r} fill={root ? tone : "#fff"} stroke={tone} strokeWidth={root ? 2 : 2.2} strokeDasharray={isCandidate(node) ? "5 3" : undefined} /><text textAnchor="middle" dy="4" fontSize={root ? "12" : "10"} fill={root ? "#fff" : tone} fontFamily="sans-serif">{nodeName(node).slice(0, 4)}</text><text textAnchor="middle" y={r + 18} fill="#253d32" fontSize="12" fontWeight={root ? "600" : "400"} fontFamily="sans-serif" paintOrder="stroke" stroke="#f7faf7" strokeWidth="4">{node.label.length > 13 ? `${node.label.slice(0, 12)}…` : node.label}</text></> : <>
                ...
                <text x={-80} y={10} fontSize="13" fontWeight="550" fill={root ? "#fff" : "#253d32"} fontFamily="sans-serif">{node.label.length > 13 ? `${node.label.slice(0, 12)}…` : node.label}</text>
              </>}
            </g>;
          })}
        </g>
      </svg>
      <div className="kg-canvas-tools"><button type="button" aria-label="放大图谱" onClick={() => zoom(1.3)}><ZoomIn size={17} /></button><button type="button" aria-label="缩小图谱" onClick={() => zoom(1 / 1.3)}><ZoomOut size={17} /></button><button type="button" aria-label="适配画布" onClick={() => setCamera(fitted(positions))}><Expand size={17} /></button><button type="button" aria-label="图谱原始比例" onClick={() => setCamera(old => ({ x: size.width / 2 - (size.width / 2 - old.x) / old.scale, y: size.height / 2 - (size.height / 2 - old.y) / old.scale, scale: 1 }))}>1:1</button>{selection && <button type="button" aria-label="清除对象高亮" onClick={() => setSelection(null)}>取消高亮</button>}<span>{Math.round(camera.scale * 100)}%</span></div>
    ...
    {navigation === "overview" && <section className="kg-domain-overview" aria-label="知识图谱领域导航"><div className="kg-domain-header"><div><strong>选择一个领域，逐层展开</strong><small>下方为界面分类，不是新增的经济关系。</small></div></div><div className="kg-domain-grid">{groups.map(group => <button type="button" key={group.key} data-domain={group.key} data-testid={`graph-domain-${group.key}`} aria-label={`查看${group.label}`} className="kg-domain-card" disabled={!data || loading} onClick={() => openCategory(group.key)}><i style={{ background: group.tone }} /><strong>{group.label}<ChevronRight size={12} /></strong><small>{group.description}</small><span>{group.key === "evidence" ? `当前返回 ${data?.evidence?.length || 0} 份原文引用` : group.coverage.map(row => `${row.label} ${row.count.toLocaleString()}`).join(" · ") || "当前范围尚未采集"}</span><small>{group.loadedCount > 0 ? `当前已载入 ${group.loadedCount} 个对象` : group.sourceCount > 0 ? "已有资料，点击按领域加载" : "点击查看数据覆盖情况"}</small></button>)}</div></section>}
    ...
    {navigation === "domain" && !!domain.records.length && <div className="kg-record-list" aria-label="本页对象列表"><small>本页对象 · 点击查看完整名称、属性和证据</small>{domain.records.map(node => <button type="button" key={node.id} className={selection?.id === node.id ? "selected" : ""} onClick={() => setSelection({ kind: "node", id: node.id })}><span style={{ color: color(node) }}>{nodeName(node)}</span><strong>{node.label}</strong>{isCandidate(node) && <em>待核实</em>}</button>)}</div>}
    </div>
