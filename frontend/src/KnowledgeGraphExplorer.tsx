...
import { ArrowLeft, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Download, Expand, Maximize2, Minimize2, Network, RefreshCw, Search, X, ZoomIn, ZoomOut } from "lucide-react";
...
export function KnowledgeGraphExplorer({ initialStock, initialCompanyId, initialGraphId, compact = false }: ExplorerProps) {
  ...
  const [domainsExpanded, setDomainsExpanded] = useState(true), [fullscreen, setFullscreen] = useState(false);
  ...
  const domainDrill = useMemo(() => {
    ...
    if (parent?.type !== "F10_SECTION") return null;
  ...
  const visible = useMemo(() => {
    ...
    if (navigation === "domain") return domainDrill || { nodes: domain.nodes, edges: domain.edges };
  ...
  }, [data, layers, hiddenTypes, navigation, identity, domain, domainDrill, allInformation]);
  const arranged = useMemo(() => navigation === "overview" ? stableDomainLayout(visible.nodes, data?.center_id || "", identity.nodes.map(node => node.id), true) : navigation === "all" || navigation === "domain" ? stableDomainLayout(visible.nodes, data?.center_id || "", identity.nodes.map(node => node.id)) : layout(visible.nodes, visible.edges, data?.center_id || ""), [visible, data?.center_id, navigation, identity.nodes]);
  ...
  function openAllInformation() { setNavigation("all"); setCategory(null); setCategoryPage(0); setCategoryType(""); setSelection(null); }
  ...
  return <section className={`knowledge-explorer kg-view-${navigation}${compact ? " kg-compact" : ""}`} aria-label="知识图谱可视化">
    ...
    {navigation === "all" && <section className="kg-hierarchy-status"><strong>全部信息 · 一级分类</strong><span>当前以{currentNode?.type === "COMPANY" ? "公司" : "证券"}为中心；点击画布中的分类节点继续下钻。</span></section>}
    <div className={`kg-layout${fullscreen ? " is-fullscreen" : ""}`} ref={layoutRef}><div className="kg-canvas-column"><div className="kg-canvas" ref={canvasRef}>
      <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" width={size.width} height={size.height} viewBox={`0 0 ${size.width} ${size.height}`} aria-label="股票公司关系画布" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => { pointer.current = null; }}>
        ...
        <g transform={`translate(${camera.x} ${camera.y}) scale(${camera.scale})`}>
          ...
          {visible.nodes.map(node => {
            ...
            const activate = () => node.type === "NAVIGATION_CATEGORY" && node.properties?.category ? openCategory(node.properties.category as GraphCategory) : setSelection({ kind: "node", id: node.id });
            return <g key={node.id} transform={`translate(${point.x} ${point.y})`} data-node-id={node.id} className="kg-node" role="button" tabIndex={0} aria-label={`${nodeName(node)}：${node.label}`} opacity={selection && !neighbors.has(node.id) ? .28 : 1} onClick={activate} onDoubleClick={() => centerNode(node)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); activate(); } }}>
      ...
      <div className="kg-canvas-tools"><button type="button" aria-label="放大图谱" onClick={() => zoom(1.3)}><ZoomIn size={17} /></button><button type="button" aria-label="缩小图谱" onClick={() => zoom(1 / 1.3)}><ZoomOut size={17} /></button><button type="button" aria-label="适配画布" onClick={() => setCamera(fitted(positions))}><Expand size={17} /></button><button type="button" aria-label={fullscreen ? "退出画布全屏" : "画布全屏"} onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={17} /> : <Maximize2 size={17} />}</button><button type="button" aria-label="图谱原始比例" onClick={() => setCamera(old => ({ x: size.width / 2 - (size.width / 2 - old.x) / old.scale, y: size.height / 2 - (size.height / 2 - old.y) / old.scale, scale: 1 }))}>1:1</button>{selection && <button type="button" aria-label="清除对象高亮" onClick={() => setSelection(null)}>取消高亮</button>}<span>{Math.round(camera.scale * 100)}%</span></div>
