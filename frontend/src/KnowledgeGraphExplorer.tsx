...
export function KnowledgeGraphExplorer({ initialStock, initialCompanyId, initialGraphId, compact = false }: ExplorerProps) {
  ...
  useEffect(() => {
    ...
    setCenter(next); setHistory([]); setHiddenTypes([]); setSearchOpen(!next);
  ...
  useEffect(() => {
    ...
    void knowledgeNetworkApi.explore({ ...center, depth, max_nodes: nodeLimit, max_edges: nodeLimit * 2, include_candidates: candidates, include_evidence: true, per_category: perCategory }, controller.signal).then(result => { if (!controller.signal.aborted) setData(result); }).catch(e => { if (!controller.signal.aborted) setError(errorText(e)); }).finally(() => { window.clearTimeout(timeout); if (!controller.signal.aborted) setLoading(false); });
  ...
  function navigate(next: NetworkQuery) { autoSelect.current = false; if (center) setHistory(old => [...old.slice(-14), center]); setCenter(next); setHiddenTypes([]); setSearchOpen(false); }
