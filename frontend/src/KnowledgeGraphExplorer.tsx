...
export function KnowledgeGraphExplorer({ initialStock, initialCompanyId, initialGraphId, compact = false }: ExplorerProps) {
  ...
  useEffect(() => { setCenter(initialStock ? { market: initialStock.market, symbol: initialStock.symbol } : initialCompanyId ? { company_id: initialCompanyId } : initialGraphId ? { graph_id: initialGraphId } : null); setHistory([]); }, [initialStock?.market, initialStock?.symbol, initialCompanyId, initialGraphId]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!center) { setData(null); return; }
    const controller = new AbortController(); setLoading(true); setError(""); setData(null); setSelection(null);
    void knowledgeNetworkApi.explore({ ...center, depth, max_nodes: nodeLimit, max_edges: nodeLimit * 2, include_candidates: candidates, include_evidence: true, per_category: perCategory }, controller.signal).then(result => { if (!controller.signal.aborted) setData(result); }).catch(e => { if (!controller.signal.aborted) setError(errorText(e)); }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [center, depth, nodeLimit, perCategory, candidates, revision]);
  ...
  }, [data, layers, hiddenTypes]);
  const arranged = useMemo(() => layout(visible.nodes, visible.edges, data?.center_id || ""), [visible, data?.center_id]);
  function fitted(points: Record<string, Point>, dimensions = size): Camera {
    ...
    const xs = values.map(p => p.x), ys = values.map(p => p.y), left = Math.min(...xs) - 95, right = Math.max(...xs) + 95, top = Math.min(...ys) - 65, bottom = Math.max(...ys) + 80;
    const scale = Math.min(1.45, Math.max(.1, Math.min(dimensions.width / (right - left), dimensions.height / (bottom - top)) * .9));
  ...
  function navigate(next: NetworkQuery) { if (center) setHistory(old => [...old.slice(-14), center]); setCenter(next); setSearchOpen(false); }
  ...
  function download(kind: "json" | "svg") {
    if (!data) return;
    const body = kind === "json" ? JSON.stringify({ ...data, display: { layers, nodes: visible.nodes.map(node => node.id), edges: visible.edges.map(edge => edge.id) } }, null, 2) : new XMLSerializer().serializeToString(svgRef.current!);
  ...
  return <section className={`knowledge-explorer${compact ? " kg-compact" : ""}`} aria-label="知识图谱可视化">
    ...
    <div className="kg-controls"><div className="kg-focus"><button type="button" title="返回上一个中心" aria-label="返回上一个中心" disabled={!history.length} onClick={() => { setCenter(history[history.length - 1]); setHistory(old => old.slice(0, -1)); }}><ArrowLeft size={15} /></button><strong>{loading ? "正在构建当前子图…" : currentNode?.label || "请选择股票或公司"}</strong>{data && <span>{visible.nodes.length} 个对象 · {visible.edges.length} 条关系</span>}</div><div className="kg-query-controls"><label>关联深度<select aria-label="关联深度" value={depth} onChange={e => setDepth(Number(e.target.value))}><option value={1}>1 层</option><option value={2}>2 层</option><option value={3}>3 层</option></select></label><label>对象上限<select aria-label="对象上限" value={nodeLimit} onChange={e => setNodeLimit(Number(e.target.value))}><option value={60}>60</option><option value={120}>120</option><option value={200}>200</option></select></label><label>每类资料<select aria-label="每类资料上限" value={perCategory} onChange={e => setPerCategory(Number(e.target.value))}><option value={3}>3 条</option><option value={5}>5 条</option><option value={10}>10 条</option></select></label><button type="button" aria-label="刷新图谱" disabled={!center || loading} onClick={() => setRevision(old => old + 1)}><RefreshCw size={15} /></button></div></div>
    <div className="kg-filters"><span>显示图层</span>{(Object.keys(layerNames) as NetworkLayer[]).map(layer => <label key={layer}><input type="checkbox" checked={layers.includes(layer)} onChange={() => setLayers(old => old.includes(layer) ? old.filter(item => item !== layer) : [...old, layer])} />{layerNames[layer]}</label>)}<label className="kg-candidate-toggle"><input type="checkbox" checked={candidates} onChange={event => setCandidates(event.target.checked)} />包含候选关系</label><label><input type="checkbox" checked={labels} onChange={event => setLabels(event.target.checked)} />关系名称</label><span className="kg-candidate-key">虚线表示待核实</span></div>
