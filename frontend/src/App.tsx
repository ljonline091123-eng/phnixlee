...
function LakehousePanel() {
  ...
  const [qualityReport, setQualityReport] = useState<LakeQualityReport | null>(null);
  ...
  const objectLabel = (item: LakeObject) => item.object_type || item.domain || sourceNames[item.source_table || ""] || item.source_table || "湖仓对象";
  const valueLabel = (value: unknown) => typeof value === "object" && value !== null ? JSON.stringify(value, null, 2) : String(value ?? "--");
  ...
  const assessQuality = async () => {
    setBusy("quality"); setError(""); setMessage("");
    try { setQualityReport(await api.assessLakeQuality({ source_table: sourceTable, layer: targetLayer, limit: 100000 })); }
    catch (e) { setError(e instanceof Error ? e.message : "质量预检失败"); } finally { setBusy(""); }
  };
  ...
  return <section className="panel lakehouse-panel">
    ...
    <div className="lakehouse-actions"><label>来源数据表<select value={sourceTable} onChange={event => { setSourceTable(event.target.value); setQualityReport(null); }}>{tables.map(table => <option key={table} value={table}>{sourceNames[table]}（{table}）</option>)}</select></label><label>目标分层<select value={targetLayer} onChange={event => { setTargetLayer(event.target.value); setQualityReport(null); }}><option value="RAW">原始层（JSONL）</option><option value="NORMALIZED">标准层（Parquet）</option><option value="SERVING">服务层（审核事实）</option></select></label><button type="button" onClick={() => void assessQuality()} disabled={Boolean(busy)}>{busy === "quality" ? "正在检查..." : "质量预检"}</button><button type="button" onClick={() => void exportDataset()} disabled={Boolean(busy)}>{busy === "export" ? "正在发布..." : "发布数据快照"}</button><button type="button" onClick={() => void archiveDocuments()} disabled={Boolean(busy)}>{busy === "archive" ? "正在归档..." : "归档知识文档"}</button></div>
    {qualityReport && <section className={`lakehouse-quality-report quality-${qualityReport.level.toLowerCase()}`}><header><div><small>质量契约 {qualityReport.contract_version}</small><strong>{qualityReport.level === "PASS" ? "通过" : qualityReport.level === "WARNING" ? "通过但有警告" : "未通过，禁止发布"}</strong></div><button type="button" aria-label="关闭质量报告" onClick={() => setQualityReport(null)}>×</button></header><div className="lakehouse-quality-grid"><div><small>检查行数</small><strong>{qualityReport.row_count.toLocaleString()}</strong></div><div><small>来源可追溯</small><strong>{qualityReport.traceable_record_count.toLocaleString()}</strong></div><div><small>重复业务键</small><strong>{qualityReport.duplicate_business_key_count}</strong></div><div><small>非法数值</small><strong>{qualityReport.invalid_numeric_count}</strong></div><div><small>量价区间问题</small><strong>{qualityReport.price_range_violation_count}</strong></div><div><small>正文缺口</small><strong>{qualityReport.missing_content_count}</strong></div></div>{qualityReport.warnings.length > 0 && <ul>{qualityReport.warnings.map(item => <li key={item}>{item}</li>)}</ul>}{qualityReport.issue_samples.length > 0 && <details><summary>查看问题样本（{qualityReport.issue_samples.length}）</summary><pre>{JSON.stringify(qualityReport.issue_samples, null, 2)}</pre></details>}</section>}
    ...
    {preview && <ResourceDialog eyebrow="数据集版本" title={`${preview.dataset} · ${preview.version}`} onClose={() => setPreview(null)}><div className="lakehouse-preview-summary"><strong>{preview.row_count.toLocaleString()} 行</strong><span className={preview.quality.passed ? "quality-pass" : "quality-fail"}>{preview.quality.passed ? "质量门禁通过" : "质量门禁未通过"}</span><span>字段 {Object.keys(preview.schema).length} 个</span></div><div className="table-wrap lakehouse-preview-table-wrap"><table><thead><tr>{Object.keys(preview.schema).slice(0, 8).map(field => <th key={field}>{field}</th>)}</tr></thead><tbody>{preview.rows.slice(0, 20).map((row, index) => <tr key={index}>{Object.keys(preview.schema).slice(0, 8).map(field => <td key={field} title={valueLabel(row[field])}>{compact(valueLabel(row[field]), 42)}</td>)}</tr>)}</tbody></table></div><details className="lakehouse-preview-details"><summary>查看字段结构和完整质量报告</summary><dl className="resource-detail-list"><dt>字段结构</dt><dd><pre>{JSON.stringify(preview.schema, null, 2)}</pre></dd><dt>质量结果</dt><dd><pre>{JSON.stringify(preview.quality, null, 2)}</pre></dd></dl></details></ResourceDialog>}
  </section>;
}
