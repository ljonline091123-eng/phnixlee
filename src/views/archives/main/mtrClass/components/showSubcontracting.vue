<template>
    <div class="container">
      ...
      <div  style="width: 50%;">
        ...
        <el-table  :data="featureList" :highlight-current-row="true" stripe @row-click="clickRow" style="width: 99%;margin-top: 8px;">
          <el-table-column width="150" label="特征项编号" align="center" prop="featureName" show-overflow-tooltip/>
          <el-table-column label="特征项名称" align="center" prop="featureCode" show-overflow-tooltip/>
          <el-table-column
            label="操作"
            v-if="isShowAdd" 
            align="center"
      ...
      <div style="width: 50%;border-left: solid 1px #E7E7E7;">
        ...
        <el-table  :data="eigenvalueList" :highlight-current-row="true"  @row-click="clickRowMainTZZ"  stripe style="width: 99%; margin-left: 1%;margin-top: 8px;">
      ...
      <div v-if="radioTypeM=='2M'"  style="width: 99%;">
        ...
        <el-table  :data="detailsList" :highlight-current-row="true" stripe  @row-click="clickRowMain"  style="width: 99%;margin-top: 8px;">
          <el-table-column width="80" label="专业分包编号" align="center" prop="majorSubcontractingCode" show-overflow-tooltip/>
          <el-table-column label="专业分包名称" align="center" prop="majorSubcontractingName" show-overflow-tooltip/>
          <el-table-column label="所属分类" align="center" prop="majorSubcontractingClassName" show-overflow-tooltip/>
      ...
      <el-dialog destroy-on-close :title="mode==='add'?'新增特征项':'编辑特征项'" :visible.sync="openFeature" width="500px" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="168px">
          ...
          <el-row>
            <el-form-item label="专业分包特征项名称:" prop="featureName" >
              <el-input v-model="form.featureName" placeholder="请输入" />
            </el-form-item>
          </el-row>
  
          <el-row>
              <el-form-item label="专业分包特征项编号:" prop="featureCode">
      ...
      <el-dialog destroy-on-close :title="modeZ==='add'?'新增特征值':'编辑特征值'" :visible.sync="openEigenValue" width="500px" append-to-body>
        <el-form ref="formZ" :model="formZ" :rules="rulesZ" label-width="168px">
          ...
          <el-row>
            <el-form-item label="专业分包特征值名称:" prop="featureValueName" >
              <el-input v-model="formZ.featureValueName" placeholder="请输入" />
            </el-form-item>
          </el-row>
  
          <el-row>
              <el-form-item label="专业分包特征值编号:" prop="featureValueCode">
      ...
      <el-dialog destroy-on-close :title="modeDetail==='add'?'新增明细':'编辑明细'" :visible.sync="openDetail" width="500px" append-to-body>
        <el-form ref="formDetail" :model="formDetail" :rules="rulesDetail" label-width="128px">
          <el-row>
            <el-form-item label="专业分包编号:" prop="majorSubcontractingCode" >
              <el-input disabled v-model="formDetail.majorSubcontractingCode" placeholder="请输入" />
            </el-form-item>
          </el-row>
          <el-row>
              <el-form-item label="专业分包名称:" prop="majorSubcontractingName">
                <el-input v-model="formDetail.majorSubcontractingName" placeholder="请输入" />
              </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="特征项及特征值:" prop="feature">
              <el-input v-model="formDetail.feature"
              type="textarea"
              maxlength="500"
              show-word-limit
              placeholder="请输入" />
        ...
        <el-row>
          <el-form-item label="基本工作内容:" prop="basicJob">
            <el-input v-model="formDetail.basicJob" placeholder="请输入"
            type="textarea"
            maxlength="2000"
            show-word-limit
            />
  ...
  <script>
    ...
    import {materialItemEdit,materialItemAdd,getMaterialItem,materialItemDelete } from "@/api/archivese/main/majorFeature";
    import {materialEigenvalueInitData,getMaterialEigenvalueList,materialEigenvalueAdd,materialEigenvalueEdit,getEigenvalue,materialEigenvalueDelete } from "@/api/archivese/main/majorValue"
    import {materialInitData} from "@/api/archivese/dossier/materialType";
    import {addMaterialDetails , updateMaterialDetails,getMaterialDetails,delMaterialDetails,initDetails } from "@/api/archivese/main/majorArchives";
  ...
  export default {
    ...
    data() {
      ...
      formDetail:{},
        ...
        openDetail:false,
      rulesDetail: {
        majorSubcontractingCode: [{required: true, message: '请输入', trigger: 'blur'}],
        majorSubcontractingName: [{required: true, message: '请输入', trigger: 'change'}],
      },
      };
    },
    computed:{
...
watch:{
  featureList: {
    ...
    getMaterialEigenvalueListFn(row){
      getMaterialEigenvalueList({majorSubcontractingFeatureId:row.id}).then(response => {
      ...
      async  featureSubmitForm(){
        ...
        if (valid) {
          ...
          this.form.majorSubcontractingClassId=this.currentNode.id
      ...
      initDetailsFn(){
        var params={}
        params.majorSubcontractingClassId=this.currentNode.id
        initDetails(params).then(res => {
          console.log(JSON.stringify(res.data.labourCode))
          this.formDetail=res.data || {}
          this.$set(this.formDetail,'majorSubcontractingCode',res.data.majorSubcontractingCode)
      ...
      async  detailSubmitForm(){
        ...
        if (valid) {
          ...
          this.formDetail.majorSubcontractingClassId=this.currentNode.id
          this.formDetail.majorSubcontractingClassName=this.currentNode.label
          console.log(JSON.stringify(this.currentNode))
      ...
      async  eigenSubmitForm(){
        ...
        if (valid) {
          ...
          this.formZ.majorSubcontractingFeatureId=this.currentFeature.id
