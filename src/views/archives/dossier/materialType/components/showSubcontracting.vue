<template>
    <div class="container">
      ...
      <div  style="width: 50%;">
        ...
        <el-table  :data="featureList" stripe  :highlight-current-row="true"  @row-click="clickRow" style="width: 99%;margin-top: 8px;">
      ...
      <div v-if="radioTypeM=='2M'"  style="width: 99%;">
        ...
        <el-table  :data="detailsList" stripe @row-click="clickRow"  :highlight-current-row="true"  style="width: 99%;margin-top: 8px;">
          <el-table-column width="150" label="专业分包编号" align="center" prop="subcontractingCode" show-overflow-tooltip/>
          <el-table-column label="专业分包名称" align="center" prop="subcontractingName" show-overflow-tooltip/>
          <el-table-column label="所属分类" align="center" prop="typeName" show-overflow-tooltip/>
          <el-table-column label="特征项及特征值" align="center" prop="itemAndEigenvalue" show-overflow-tooltip/>
          ...
          <el-table-column label="基本工作内容" align="center" prop="workContent" show-overflow-tooltip/>
          <el-table-column
          v-if="isShowAdd" 
            label="操作"
            align="center"
      ...
      <el-dialog destroy-on-close :title="mode==='add'?'新增特征项': mode==='view'?'查看特征项':'编辑特征项'" :visible.sync="openFeature" width="500px" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="168px">
         
          <el-row>
            <el-form-item label="专业分包特征项名称:" prop="itemName" >
          ...
          <el-row>
              <el-form-item label="专业分包特征项编号:" prop="itemCode">
      ...
      <el-dialog destroy-on-close :title="modeZ==='add'?'新增特征值': modeZ==='view'?'查看特征值':'编辑特征值'" :visible.sync="openEigenValue" width="500px" append-to-body>
        <el-form ref="formZ" :model="formZ" :rules="rulesZ" label-width="168px">
         
          <el-row>
            <el-form-item label="专业分包特征值名称:" prop="eigenvalueName" >
              <span v-if="mode === 'view'">{{formZ.eigenvalueName}}</span>
              <el-input v-else v-model="formZ.eigenvalueName" placeholder="请输入" />
            </el-form-item>
          </el-row>
  
          <el-row>
              <el-form-item label="专业分包特征值编号:" prop="eigenvalueCode">
                <span v-if="mode === 'view'">{{formZ.eigenvalueCode}}</span>
      ...
      <el-dialog destroy-on-close :title="modeDetail==='add'?'新增明细': modeDetail==='view'?'查看明细':'编辑明细'" :visible.sync="openDetail" width="500px" append-to-body>
        <el-form ref="formDetail" :model="formDetail" :rules="rulesDetail" label-width="128px">
          ...
          <el-row>
            <el-form-item label="专业分包编号:" prop="subcontractingCode" >
              <span v-if="modeDetail === 'view'">{{formDetail.subcontractingCode}}</span>
              <el-input v-else  disabled v-model="formDetail.subcontractingCode" placeholder="请输入" />
            </el-form-item>
          </el-row>
  
          <el-row>
              <el-form-item label="专业分包名称:" prop="subcontractingName">
                <span v-if="modeDetail === 'view'">{{formDetail.subcontractingName}}</span>
                <el-input  v-else  v-model="formDetail.subcontractingName" placeholder="请输入" />
              </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="特征项及特征值:" prop="itemAndEigenvalue">
              <span v-if="modeDetail === 'view'">{{formDetail.itemAndEigenvalue}}</span>
              <el-input v-else  v-model="formDetail.itemAndEigenvalue" placeholder="请输入"  type="textarea"
              maxlength="500"
              show-word-limit/>
            </el-form-item>
        </el-row>
      ...
      <el-row>
        <el-form-item label="计量规则:" prop="measurementRules">
          <span v-if="modeDetail === 'view'">{{formDetail.measurementRules}}</span>
          <el-input v-else  v-model="formDetail.measurementRules" placeholder="请输入" />
        </el-form-item>
    </el-row>
    <el-row>
      <el-form-item label="基本工作内容:" prop="workContent">
        <span v-if="modeDetail === 'view'">{{formDetail.workContent}}</span>
        <el-input v-else  v-model="formDetail.workContent" placeholder="请输入"  type="textarea"
        maxlength="2000"
        show-word-limit/>
      </el-form-item>
  </el-row>
  ...
  <script>
    ...
    import {materialItemEdit,materialItemAdd,getMaterialItem,materialItemDelete } from "@/api/archivese/dossier/subcontractingItem";
    import {materialEigenvalueInitData,getMaterialEigenvalueList,materialEigenvalueAdd,materialEigenvalueEdit,getEigenvalue,materialEigenvalueDelete } from "@/api/archivese/dossier/subcontractingEigenvalue"
    import {materialInitData} from "@/api/archivese/dossier/materialType";
    import {addMaterialDetails , updateMaterialDetails,getMaterialDetails,delMaterialDetails ,initDetails} from "@/api/archivese/dossier/subcontractingDetails";
  ...
  export default {
    ...
    data() {
      ...
      formDetail:{},
        ...
        openDetail:false,
      rulesDetail: {
        subcontractingCode: [{required: true, message: '请输入', trigger: 'blur'}],
        subcontractingName: [{required: true, message: '请输入', trigger: 'change'}],
      },
      };
    },
    computed:{
  ...
  featureList: {
    ...
    getMaterialEigenvalueListFn(row){
      ...
      initDetailsFn(){
        ...
        params.typeId=this.currentNode.id
        initDetails(params).then(res => {
          this.formDetail=res.data || {}
          this.$set(this.formDetail,'subcontractingCode',res.data.subcontractingCode)
          })
      },
      //明细
      detailAdd(){
