<template>
    <div class="container">
      ...
      <div v-if="radioTypeM=='2M'"  style="width: 99%;">
        ...
        <el-table  :data="detailsList" stripe @row-click="clickRow"  :highlight-current-row="true"  style="width: 99%;margin-top: 8px;">
          <el-table-column width="150" label="劳务编号" align="center" prop="labourCode" show-overflow-tooltip/>
          <el-table-column label="劳务名称" align="center" prop="labourName" show-overflow-tooltip/>
      ...
      <el-dialog destroy-on-close :title="mode==='add'?'新增特征项': mode==='view'?'查看特征项':'编辑特征项'" :visible.sync="openFeature" width="500px" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="128px">
          ...
          <el-row>
            <el-form-item label="劳务特征项名称:" prop="itemName" >
          ...
          <el-row>
              <el-form-item label="劳务特征项编号:" prop="itemCode">
      ...
      <el-dialog destroy-on-close :title="modeZ==='add'?'新增特征值': modeZ==='view'?'查看特征值':'编辑特征值'" :visible.sync="openEigenValue" width="500px" append-to-body>
        <el-form ref="formZ" :model="formZ" :rules="rulesZ" label-width="128px">
          ...
          <el-row>
            <el-form-item label="劳务特征值名称:" prop="eigenvalueName" >
          ...
          <el-row>
              <el-form-item label="劳务特征值编号:" prop="eigenvalueCode">
      ...
      <el-dialog destroy-on-close :title="modeDetail==='add'?'新增明细': modeDetail==='view'?'查看明细':'编辑明细'" :visible.sync="openDetail" width="500px" append-to-body>
        <el-form ref="formDetail" :model="formDetail" :rules="rulesDetail" label-width="128px">
          ...
          <el-row>
            <el-form-item label="劳务编号:" prop="labourCode" >
              <span v-if="modeDetail === 'view'">{{formDetail.labourCode}}</span>
              <el-input v-else disabled v-model="formDetail.labourCode" placeholder="请输入" />
            </el-form-item>
          </el-row>
  
          <el-row>
              <el-form-item label="劳务名称:" prop="labourName">
                <span v-if="modeDetail === 'view'">{{formDetail.labourName}}</span>
                <el-input v-else v-model="formDetail.labourName" placeholder="请输入" />
              </el-form-item>
          </el-row>
          <el-row>
  ...
  <script>
    ...
    import {materialItemEdit,materialItemAdd,getMaterialItem,materialItemDelete } from "@/api/archivese/dossier/labourItem";
    import {materialEigenvalueInitData,getMaterialEigenvalueList,materialEigenvalueAdd,materialEigenvalueEdit,getEigenvalue,materialEigenvalueDelete } from "@/api/archivese/dossier/labourEigenvalue"
    import {materialInitData} from "@/api/archivese/dossier/materialType";
    import {addMaterialDetails , updateMaterialDetails,getMaterialDetails,delMaterialDetails,initDetails } from "@/api/archivese/dossier/labourDetails";
  ...
  export default {
    ...
    data() {
      ...
      formDetail:{},
        ...
        //明细
      rulesDetail: {
        labourName: [{required: true, message: '请输入', trigger: 'blur'}],
        labourName: [{required: true, message: '请输入', trigger: 'change'}],
      },
      };
    },
    computed:{
    ...
    getMaterialEigenvalueListFn(row){
      ...
      initDetailsFn(){
        ...
        initDetails(params).then(res => {
          ...
          this.$set(this.formDetail,'labourCode',res.data.labourCode)
          })
      },
      //明细
      detailAdd(){
