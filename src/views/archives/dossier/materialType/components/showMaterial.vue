<template>
    <div class="container">
      ...
      <div v-if="radioTypeM=='2M'"  style="width: 99%;">
        ...
        <el-table  :data="detailsList" stripe  :highlight-current-row="true"  style="width: 99%;margin-top: 8px;">
          <el-table-column width="150" label="材料编号" align="center" prop="materialCode" show-overflow-tooltip/>
          ...
          <el-table-column label="规格" align="center" prop="materialSpecifications" show-overflow-tooltip/>
        ...
        <el-form ref="formDetail" :model="formDetail" :rules="rulesDetail" label-width="128px">
          ...
          <el-row>
            <el-form-item label="材料编号:" prop="materialCode" >
              <span v-if="modeDetail === 'view'">{{formDetail.materialCode}}</span>
              <el-input  v-else  v-model="formDetail.materialCode" placeholder="请输入" />
            </el-form-item>
          </el-row>
  
          <el-row>
              <el-form-item label="材料名称:" prop="materialName">
                <span v-if="modeDetail === 'view'">{{formDetail.materialName}}</span>
                <el-input v-else v-model="formDetail.materialName" placeholder="请输入" />
              </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="规格:" prop="materialSpecifications">
              <span v-if="modeDetail === 'view'">{{formDetail.materialSpecifications}}</span>
              <el-input v-else v-model="formDetail.materialSpecifications" placeholder="请输入" />
            </el-form-item>
        </el-row>
        <el-row>
  ...
  <script>
    ...
    import {materialItemEdit,materialItemAdd,getMaterialItem,materialItemDelete } from "@/api/archivese/dossier/file/materialItem";
    import {materialEigenvalueInitData,getMaterialEigenvalueList,materialEigenvalueAdd,materialEigenvalueEdit,getEigenvalue,materialEigenvalueDelete } from "@/api/archivese/dossier/file/materialEigenvalue"
    import {materialInitData} from "@/api/archivese/dossier/materialType";
    import {addMaterialDetails , updateMaterialDetails,getMaterialDetails,delMaterialDetails } from "@/api/archivese/dossier/file/materialDetails";
...
watch:{
  featureList: {
    ...
    getMaterialEigenvalueListFn(row){
      ...
      //this.orgId有值表示待审库的责任单位 this.org默认单位
