                  ...
                  <template slot-scope="inventory">
                    ...
                    <el-table
                      ...
                      <el-table-column label="工作量" align="right" prop="count" width="150" v-if="currentContract.contractPlanningCategory == 2 || currentContract.contractPlanningCategory == 3">
                        <template slot-scope="scope">
                          <el-input v-model="scope.row.count" :disabled="isSubmit" v-if="scope.row.rentMode == 3"  v-thousandth/>
                          <span v-else>{{ scope.row.count || 0 }}</span>
                        </template>
                      </el-table-column>
                      <el-table-column label="清单数量" align="right" prop="count" width="150" v-else>
                        <template slot-scope="scope">
                          <el-input title="清单数量" v-model="scope.row.count" :disabled="isSubmit || scope.row.belongOffer || scope.row.pushFlag === 'Y'"  v-thousandth/>
                        </template>
                      </el-table-column>
...
export default {
  ...
  methods: {
        ...
        this.$refs[formName].validate(async (valid,done) => {
          if (valid) {
            ...
            // const {contractPlanningName, plannedAmountInclTaxText, bidResponsibleOrgName} = this.currentContract
                    // 获取用户填写的合约名称和计划金额
            const userModifiedData = this.planList[0]?.children.map(item => (
              {
                contractPlanningName: item.contractPlanningName, // 用户填写的合约名称
                plannedAmountInclTaxText: item.plannedAmountInclTaxText, // 用户填写的计划金额
                bidResponsibleOrgName: item.contractPlanningName,
              }
            ))
              ...
              if (obj.hasOwnProperty(key)) {
                ...
                if (targetKeys.includes(key)) {
                  if(typeof obj[key] === "string"){
                    // 字符直接替换
                    const tempObj = obj[key].replaceAll(/,/g, '');
                    newObj[key] = tempObj;
                  } else if (typeof obj[key] === "object" && Array.isArray(obj[key])) {
                    // 如果是数组，递归处理
                    const tempObj = this.removeThousandsSeparator(obj[key], targetKeys);
                    newObj[key] = tempObj;
    ...
    checkOtherPrice(event,row) {
      const regexN1 = /^-?(?:[1-9]\d*|0)(\.\d+)?$/;
      const regexN2 = /^-?\d+(\.\d{0,4})?$/;
      if(!regexN2.test(row.basePrice)){
        event.target.style = "border: 1px solid red;"
        this.$message.error("请输入小于4位的小数");
        this.$set(row, `isbasePriceNotLegal`, true)
        return;
      }else {
        event.target.style = "border: 1px solid #C0C4CC;"
  ...
  computed: {
    ...
    getUnitPriceInclTax() {
      return ( row ) => {
        /* 计算 含税单价 不含税单价 行合计价 */
        this.calculatePrice(row);
        return row.unitPriceInclTax ? (row.unitPriceInclTax) : (0.00);
      }
    },
    /* 不含税单价 */
    getUnitPriceExclTax() {
