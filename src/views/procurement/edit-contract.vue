        ...
        <el-tab-pane label="基本信息" name="first">
            ...
            <el-row :gutter="10">
              ...
              <el-col :span="8">
                <el-form-item label="支付方式：" prop="agreement.paymentWay"
                  ...
                  <el-select style="width: 100%" multiple v-model="paymentWayArr" placeholder="请选择支付方式">
            ...
            <el-row :gutter="10" v-if="[4, 5].includes(contractType)">
              ...
              <el-col :span="8">
                <el-form-item label="工期(天)" prop="agreement.duration">
                  <el-input :value="firstForm.duration || durationComputed(firstForm.agreement.entryDate, firstForm.agreement.finishDate)" placeholder="请输入工期" disabled >
          ...
          <div style="margin-bottom: 8px">
            ...
            <el-row :gutter="10">
              ...
              <el-col :span="8">
                <el-form-item label="约定预付款金额：" prop="agreementPaymentItem.prepaymentAmount"
                  :rules="[{validator: validateNumber, trigger: 'blur'}]">
          ...
          <div style="margin-bottom: 24px" v-if="contractType == 1">
            <el-table :data="firstForm.agreementMaterialsLists" style="width: 100%">
              ...
              <el-table-column prop="signCount" label="本次签订量" width="130" align="right">
                <template slot-scope="scope">
                  <el-form-item label-width="0" :prop="'agreementMaterialsLists.' + scope.$index + '.signCount'">
                    <el-input v-model="scope.row.signCount"  v-thousandth clearable @blur="computedAmount(scope)"/>
                  </el-form-item>
                </template>
              </el-table-column>
              <el-table-column prop="signUnitPriceInclTax" label="本次签订含税单价(元)" width="160" align="right">
                <template slot-scope="scope">
                  <el-form-item label-width="0" :prop="'agreementMaterialsLists.' + scope.$index + '.signUnitPriceInclTax'">
                    <el-input
                      v-model="scope.row.signUnitPriceInclTax"
                      placeholder="请输入"
                      @blur="computedAmount(scope)"
                      v-thousandth
                    />
                  </el-form-item>
                </template>
              ...
              <el-table-column prop="signTaxAmountText" label="本次税额(元)" width="150" align="right"/>
              <el-table-column prop="notTaxUnitPriceText" label="单价(不含税)" v-if="priceType != 2" width="120" align="right"/>
              <el-table-column prop="taxUnitPriceText" label="单价(含税)" v-if="priceType != 2" width="120" align="right"/>
              ...
              <el-table-column prop="rentalUnit" align="center" label="计租单位" width="120">
                <template slot-scope="scope">
                  <el-form-item label-width="0" :prop="'agreementMaterialsLists.' + scope.$index + '.rentalUnit'"
                                :rules="[{ required: true, trigger: 'change', message: '请选择计' }]">
          ...
          <div style="margin-bottom: 24px" v-if="contractType == 6">
            <el-table :data="firstForm.agreementMaterialsLists" style="width: 100%">
              ...
              <el-table-column prop="taxRateText" label="税率(%)" align="center" />
              <el-table-column prop="taxAmountText" label="税额" />
              <el-table-column prop="remark" align="center" width="180" label="备注">
                <template slot-scope="scope">
                  <el-form-item :prop="'agreementMaterialsLists.' + scope.$index + '.remark'" label-width="0">
                    <el-input v-model="scope.row.remark" clearable />
          ...
          <div style="margin-bottom: 24px;" v-if="[4, 5].includes(contractType)">
            ...
            <div class="engineering_visa">
              ...
              <el-table :data="firstForm.agreementDailyWageList" style="width: 100%">
                ...
                <el-table-column prop="unitPriceIncTax" align="center" label="单价(含税)">
                  <template slot-scope="scope">
                ...
                <el-table-column prop="unitPriceExcTax" align="center" label="单价(不含税)">
                  <template slot-scope="scope">
          ...
          <div style="margin-bottom: 24px;" v-if="contractType == 4">
            ...
            <el-table :data="firstForm.agreementEquipmentSupplies" style="width: 100%">
              ...
              <el-table-column prop="estimatedUnitPriceExcTax" align="center" label="预估单价(不含税)">
                <template slot-scope="scope">
                  {{ countComputed(scope.row.estimatedUnitPriceIncTax, scope.row.estimatedTaxRate, scope.row.estimatedCount, 'excludingTax') }}
                </template>
              </el-table-column>
              <el-table-column prop="estimatedUnitPriceIncTax" align="center" label="预估单价(含税)">
                <template slot-scope="scope">
                  <el-form-item label-width="0"
          ...
          <div style="margin-bottom: 24px;" v-if="contractType == 4">
            ...
            <el-table :data="firstForm.agreementMaterialSupplies" style="width: 100%">
              ...
              <el-table-column prop="estimatedUnitPriceExcTax" label="预估单价(不含税)">
                <template slot-scope="scope">
                  {{ countComputed(scope.row.estimatedUnitPriceIncTax, scope.row.estimatedTaxRate, scope.row.estimatedCount, 'excludingTax') }}
                </template>
              </el-table-column>
              ...
              <el-table-column prop="estimatedAmountExcTax" label="预估金额(不含税)">
                <template slot-scope="scope">
                  {{ countComputed(scope.row.estimatedUnitPriceIncTax, scope.row.estimatedTaxRate, scope.row.estimatedCount, 'excludingTaxTotal') }}
                </template>
              </el-table-column>
              <el-table-column prop="estimatedAmountIncTax" label="预估金额(含税)">
                <template slot-scope="scope">
                  {{ countComputed(scope.row.estimatedUnitPriceIncTax, scope.row.estimatedTaxRate, scope.row.estimatedCount, 'taxIncludedTotal') }}
                </template>
              </el-table-column>
              <el-table-column prop="estimatedTaxAmount" label="预估税额">
                <template slot-scope="scope">
                  {{ countComputed(scope.row.estimatedUnitPriceIncTax, scope.row.estimatedTaxRate, scope.row.estimatedCount) }}
                </template>
              </el-table-column>
              ...
              <el-table-column prop="returnCondition" label="返还条件">
                <template slot-scope="scope">
                  <el-form-item label-width="0" :prop="'agreementDeposits.' + scope.$index + '.returnCondition'"
                    ...
                    <el-input v-model="scope.row.returnCondition" clearable type="textarea" :autosize="{maxRows: 2}"/>
                  </el-form-item>
                </template>
              </el-table-column>
              <el-table-column prop="remark" label="备注">
                <template slot-scope="scope">
                  <el-form-item :prop="'agreementDeposits.' + scope.$index + '.remark'" label-width="0">
                    <el-input v-model="scope.row.remark" clearable type="textarea" :autosize="{maxRows: 2}"/>
                  </el-form-item>
                </template>
              </el-table-column>
              <el-table-column label="操作" align="center" width="80">
  ...
  methods: {
    ...
    getAgreementDetail(){
        ...
        id: this.id,
           ...
           // 合同类型
          this.contractType = res.data.agreement.expenditureBusinessType
          this.priceType = res.data.agreement.priceType

          this.firstForm.agreement = res.data.agreement || {} // 合同基本信息
          this.firstForm.agreementDailyWageList=res.data.agreementDailyWageList || []// 合同-计日工对象
          this.firstForm.agreementDeposits = res.data.agreementDeposits || [] // 合同保证金
          this.firstForm.agreementEquipmentSupplies = res.data.agreementEquipmentSupplies || [] // 合同-甲供材料清单对象
    ...
    subForm(value){
      ...
      if(this.isSubmit){
        ...
        let params = JSON.parse(JSON.stringify(this.firstForm))
    ...
    computedpaymentSAmount(scope){
      ...
      const totalAmountIncTax = this.firstForm?.agreement?.totalAmountIncTax
    ...
    computedAmount(scope) {
        ...
        if ( !signUnitPriceInclTax || !signCount || !signTaxRate ) return
        ...
        const { add, divide, multiply, bignumber, subtract } = this.mathjs;

        const taxUnitPriceBig = bignumber(signUnitPriceInclTax);
        const taxRateBig = bignumber(signTaxRate);
        const countBig = bignumber(signCount);
        ...
        const signUnitPriceExclTaxText = this.formatted(signUnitPriceInclTax, notTaxedPrice, 'excludingTax')
        const signAmountInclTaxText = this.formatted(signUnitPriceInclTax, taxedTotal)
        const signAmountExclTaxText = this.formatted(signUnitPriceInclTax, notTaxedTotal)
        const signTaxAmountText = this.formatted(signUnitPriceInclTax, taxAmount)

        this.$set(scope.row, 'signUnitPriceExclTaxText', signUnitPriceExclTaxText)
        this.$set(scope.row, 'signAmountInclTaxText', signAmountInclTaxText)
        this.$set(scope.row, 'signAmountExclTaxText', signAmountExclTaxText)
        this.$set(scope.row, 'signTaxAmountText', signTaxAmountText)

    },
    formatted(taxUnitPrice, result, type){
      const { format } = this.mathjs;
      let formattedResult;

      let newRes = taxUnitPrice.toString().replace(/\.?0+$/, '')
      console.log(newRes,'含税去小数', type);
