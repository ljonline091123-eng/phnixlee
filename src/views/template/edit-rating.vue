<template>
  <div class="app-container">
    <BackButton path="/template/template" title="修改评分模板">
      <div class="page-title-right">
        <el-button
          ...
          size="mini"
          :disabled="isSubmit"
          @click="$router.replace('/template/template')"
          >取消</el-button
        >
        <el-button
          type="primary"
    ...
    "$route.params.params": {
      async handler(val) {
        if (val) {
          ...
          this.unitId = res.data.useUnit;
          res.data.selectedTypes = res.data.markCategoryDatailVOList.map(
            (obj) => String(obj.itemType)
          );
          ...
          res.data.biddingMarkCategoryVOList =
            res.data.markCategoryDatailVOList.map((obj, index) => {
              ...
              res.data.markCategoryDatailVOList[index].biddingMarkItemVOList =
                obj.markItemDetailVOList.map((item) => ({
                  ...item,
                  subBiddingMarkItemVOList: item.subBiddingMarkItemDetailVOList,
                }));
              return obj;
            });
