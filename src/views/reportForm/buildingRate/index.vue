        ...
        {prop: 'gkNum', label: '公开次数',align: 'right',clickMethod:(row)=>{
            if (row.type === "G") {
              getProjectCode(row.id).then(res=>{
                ...
                window.$wujie?.bus.$emit('routeChange', {
                path: `/zbcg/procurement/procurement$bindding?projectCodeList=`+res.data+`&type=buildingRate&noticeStatus=8&procurementType=1&report=report`,
                query:{projectCodeList:res.data,type:'buildingRate',noticeStatus:'8',procurementType:'1',report:'report'}
              })
              })
            } else {
              ...
              window.$wujie?.bus.$emit('routeChange', {
                path: `/zbcg/procurement/procurement$bindding?projectCode=`+row.id+`&type=buildingRate&noticeStatus=8&procurementType=1&report=report`,
                query:{projectCode:row.id,type:'buildingRate',noticeStatus:'8',procurementType:'1',report:'report'}
              })
            }
          }},
