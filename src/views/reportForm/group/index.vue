...
export default {
  ...
  methods: {
    ...
    // 显示所有父节点（parentId === '0'），关闭子节点和孙节点
    // showParentNodes() {
    //   const tree = this.$refs.tree;

    //   Object.values(tree.store.nodesMap).forEach((node) => {
    //     if (node.data.parentId === "0") {
    //       tree.store.getNode(node.key).expanded = false; // 父节点保持收起状态
    //     } else {
    //       tree.store.getNode(node.key).expanded = false; // 子节点和孙节点全部收起
    //     }
    //   });
    // },
    // expandChildNodes() {
    ...
    //     if (node.data.parentId === "0") {
    //       // 如果是父节点
    //       tree.store.getNode(node.key).expanded = true; // 展开
    //     } else {
    //       // 收起子节点和孙节点
    //       tree.store.getNode(node.key).expanded = false; // 收起
    //     }
    //   });
    // },
    // 展开所有父节点、子节点、孙节点
    // expandGrandchildNodes() {
    //   const tree = this.$refs.tree;

    //   Object.values(tree.store.nodesMap).forEach((node) => {
    //     tree.store.getNode(node.key).expanded = true; // 展开所有节点
    //   });
    // },
    // 判断节点是否为子节点：parentId 是否为父节点的 deptId
    // isChildNode(node) {
    //   // 找到父节点并检查它的 deptId
    //   return Object.values(this.$refs.tree.store.nodesMap).some(
    //     (parentNode) => {
    //       return parentNode.data.deptId === node.data.parentId;
    //     }
    //   );
    // },
    // 展开到指定层级，并收起超出层级的节点
    ...
    getDeptTree() {
      ...
      deptTree({
        ...
        .then((response) => {
          ...
          console.log(this.deptOptions, "this.deptOptions-this.deptOptions");
          // * 默认取第一个，查询表格
