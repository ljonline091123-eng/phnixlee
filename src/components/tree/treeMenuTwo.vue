<template>
  <div class="left">
    ...
    <!--<div v-if="queryTypeTwo" style="   width: 100%;height: 33px;margin: -10px 10px 16px 0; background-color: #ffffff;">
      <el-radio-group
        style="margin:0px 0 0 10px;"
        v-model="queryRadioTypeTwo"
        size="small"
        @change="handleRadioChangeTwo"
        disabled="true"
      >
        <el-radio-button
      ...
      handleNodeClick(data,node,prop) {
        ...
        if( this.preNodeId && this.nodeCount >= 2){
          ...
          if(this.curNodeId == this.preNodeId){//第一次点击的节点和第二次点击的节点id相同
            ...
            debugger;
            if(!data.state){
              this.$message.error("该节点不能选择，请选择具体档案的节点！");
              return
            }
