...
def explore(db: Session, *, market=None, symbol=None, company_id=None, center_id=None, graph_id=None,
            depth=1, max_nodes=160, max_edges=300, include_candidates=False, include_evidence=True, per_category=3):
    ...
    for target in targets:
        part = build_dynamic_projection(db, target.market, target.symbol, company_id=company.id if company else None,
            per_category=per_category, include_evidence=include_evidence, graph_id=graph_id)
        p.merge(part)
    ...
    if include_evidence:
        ...
        p.edges = {key: edge for key, edge in p.edges.items() if edge.get("layer") != "EVIDENCE"}
    ...
    coverage = [{"key": key, "label": label, "status": "PRESENT" if count else "NOT_COLLECTED", "count": count} for key, label, count in dimensions]
    ...
    return {"center_id": root_id, "centers": centers, "nodes": nodes, "edges": edges, "evidence": evidence,
        "coverage": coverage, "stats": {"nodes": len(nodes), "edges": len(edges), "evidence": len(evidence),
        ...
        "limits": {"max_nodes": max_nodes, "max_edges": max_edges, "per_category": per_category, "depth": depth},
