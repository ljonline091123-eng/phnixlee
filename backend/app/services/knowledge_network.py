...
def _scope_category(p, category, root_id, issuer_id, include_evidence):
    ...
    for node in p.nodes.values():
        ...
        if category == "disclosure" and props.get("source_table") in DISCLOSURE_TABLES and props.get("document_type") != "F10_SOURCE":
    ...
    if category == "disclosure":
        # F10 items are a third navigation level. Keep them available in the
        # response, but omit them from category_node_ids and the first page.
        section_ids = {node_id for node_id in selected if p.nodes.get(node_id, {}).get("type") == "F10_SECTION"}
        f10_edges = [edge for edge in p.edges.values() if edge["source"] in section_ids and edge["type"] == "HAS_F10_ITEM"]
        keep.update(edge["target"] for edge in f10_edges)
        keep.update(edge["source"] for edge in f10_edges)
