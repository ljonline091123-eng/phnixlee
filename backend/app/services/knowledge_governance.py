...
def _review(db: Session, agent: AgentDefinition, skill: ModelSkill, request: KnowledgeGovernanceRequest,
    ...
    if not log.response_text and any(choice.get("finish_reason") == "length" for choice in (log.response_json or {}).get("choices", [])):
        report["model_status"] = "OUTPUT_LIMIT_REACHED"
        raise ValueError("模型输出额度已用完，但未生成最终 JSON；本批次未发布，请调整模型输出额度后重试。")
