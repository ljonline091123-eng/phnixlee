...
F10_LABELS = {
    ...
    "shareholder": "股东资料", "holders": "主要股东", "business": "经营资料", "business_analysis": "经营分析",
    "dividends": "分红资料", "capital": "股本资料", "industry": "行业资料",
    "published_reports": "已披露报告", "fund_flow": "资金流向", "company_events": "公司事项",
}


def _f10_lists(payload: dict[str, Any], prefix: str = "", depth: int = 0) -> list[tuple[str, list[Any]]]:
    ...
    for key, value in payload.items():
        ...
        if isinstance(value, list):
            ...
            result.extend(_f10_lists(value, path, depth + 1))
    return result


def f10_content_profile(section: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Expose concrete F10 contents without inventing facts from opaque fields."""
    label = F10_LABELS.get(section.lower(), section if re.search(r"[\u4e00-\u9fff]", section) else "其他资料")
    collections = _f10_lists(payload)
    item_count = sum(len(value) for _, value in collections)
    fields = payload.get("fields") if isinstance(payload.get("fields"), dict) else {}
    period = next((str(value) for key in ("report_date", "notice_date", "date", "period")
        if (value := payload.get(key)) not in (None, "")), None)
    contents = [f"{key} {len(value)} 条" for key, value in collections]
    if fields:
        contents.append(f"资料字段 {len(fields)} 项")
    if not contents:
        contents.append(f"原始字段 {len(payload)} 项")
    return {"section": section, "section_label": label, "item_count": item_count,
        "field_count": len(fields), "content_overview": "；".join(contents), "latest_period": period,
        "available_content_groups": [key for key, _ in collections] + (["fields"] if fields else [])}


def f10_concrete_items(payload: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    """Return bounded source items; labels use only fields actually supplied."""
    result: list[tuple[str, dict[str, Any]]] = []
    for group, values in _f10_lists(payload):
        for index, value in enumerate(values[:8]):
            props = _json_safe(value if isinstance(value, dict) else {"value": value})
            label = next((str(props[key]) for key in ("title", "report_name", "name", "report_type", "report_date", "notice_date", "date")
                if props.get(key) not in (None, "")), None)
            result.append((label or f"{group} 第 {index + 1} 条", {"content_group": group, "item_index": index + 1, **props}))
    return result[:12]
...
def build_dynamic_projection(db: Session, market: str, symbol: str, *, company_id: str | None = None,
    ...
    for row in recent(StockF10Cache, StockF10Cache.fetched_at, "f10"):
        profile = f10_content_profile(row.section, row.payload_json or {})
        label = profile["section_label"]
        evidence_id = result.document(row, f"{symbol} F10 {label}", row.payload_json, "F10_SOURCE")
        section_id = result.fact(row, "section", "F10_SECTION", f"F10 · {label}", "F10资料分区",
            "HAS_F10_SECTION", "包含F10资料", profile, evidence_id)
        for index, (item_label, item) in enumerate(f10_concrete_items(row.payload_json or {}), start=1):
            item_id = f"dynamic:{row.__tablename__}:{row.id}:item:{index}"
            item_props = {"market": market, "symbol": symbol, "section": row.section, "section_label": label,
                "source_table": row.__tablename__, "source_record_id": row.id, "source_name": result.evidence[evidence_id]["source_name"],
                "observed_at": result.evidence[evidence_id]["observed_at"], **item}
            result.node(item_id, "F10_ITEM", item_label, "F10具体资料", item_props,
                status="NORMALIZED", evidence_ids=[evidence_id])
            result.edge(section_id, item_id, "HAS_F10_ITEM", "包含具体资料", evidence_id)
            if include_evidence:
                result.edge(item_id, evidence_id, "SUPPORTED_BY", "证据支持", evidence_id, layer="EVIDENCE")
        if include_evidence:
            result.nodes[evidence_id]["properties"].update(profile)
