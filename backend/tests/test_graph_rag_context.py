...
def test_context_contains_structured_observations_and_precise_lakehouse_lineage():
    ...
    with Session(engine) as db:
        ...
        db.add_all([
            LakeLineageEvent(
                batch_id=f"unrelated-{index}", upstream_type="LAKE_OBJECT", upstream_id=f"other-{index}",
                downstream_type="DOCUMENT_CHUNK", downstream_id=f"other-chunk-{index}",
                transformation="DOCUMENT_CHUNKING", created_at=now + timedelta(hours=2),
            )
...
def test_context_excludes_future_graph_relations_and_reports_prompt_chunk_budget():
    ...
    with Session(engine) as db:
        ...
        assert context["graph_paths"][0]["verification_status"] == "OBSERVED"
        assert context["graph_paths"][0]["extraction_method"] == "SYSTEM"
        assert "confidence" in context["graph_paths"][0]
...
def test_context_applies_cutoff_before_structured_query_limits():
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    cutoff = datetime(2026, 9, 25, tzinfo=timezone.utc)
    past = cutoff - timedelta(days=1)
    ...
    with Session(engine) as db:
        source = DataSource(source_code="CUTOFF_SOURCE", source_name="时点压力来源", adapter_type="TEST")
        db.add(source)
        db.flush()
        past_financial = StockFinancialReport(
            market="CN_A", symbol="000001", indicator="past_revenue", report_period="2026Q2",
            data_json={"value": 100}, source_id=source.id, fetched_at=past,
        )
        past_bar = StockKline(
            market="CN_A", symbol="000001", period="daily", adjust="", trade_date="2026-09-24",
            close_price=10, volume=100, source_id=source.id, fetched_at=past,
        )
        past_f10 = StockF10Cache(
            market="CN_A", symbol="000001", section="主要股东", source_id=source.id,
            payload_json={"持股比例": 0.2}, fetched_at=past,
        )
        db.add_all([past_financial, past_bar, past_f10])
        db.flush()
        past_ids = {past_financial.id, past_bar.id, past_f10.id}
        for index in range(30):
            db.add(StockFinancialReport(
                market="CN_A", symbol="000001", indicator=f"future_metric_{index}",
                report_period=f"2099Q{index:02d}", data_json={"value": index},
                source_id=source.id, fetched_at=future + timedelta(minutes=index),
            ))
            db.add(StockKline(
                market="CN_A", symbol="000001", period="daily", adjust="",
                trade_date=(future + timedelta(days=index)).date().isoformat(),
                close_price=20 + index, volume=200 + index, source_id=source.id,
                fetched_at=future + timedelta(minutes=index),
            ))
            db.add(StockF10Cache(
                market="CN_A", symbol="000001", section=f"股东资料 {index}", source_id=source.id,
                payload_json={"future": True}, fetched_at=future + timedelta(minutes=index),
            ))
        db.commit()

        context = build_selection_context(db, "CN_A", "000001", as_of=cutoff, max_facts=2)
        observations = context["structured_observations"]
        selected_ids = {
            row["source_record_id"]
            for rows in observations.values()
            for row in rows
        }
        assert past_ids <= selected_ids
        assert all(row["observed_at"] <= cutoff.isoformat() for rows in observations.values() for row in rows)
    engine.dispose()
