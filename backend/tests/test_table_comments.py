...
class TableCommentTests(unittest.TestCase):
    def test_every_model_table_has_a_description(self) -> None:
        ...
        self.assertEqual(len(TABLE_DESCRIPTIONS), 35)
        ...
        self.assertEqual(sum(len(table.columns) for table in Base.metadata.tables.values()), 395)
    ...
    def test_catalog_is_queryable_and_seed_is_idempotent(self) -> None:
        ...
        with Session(engine) as session:
            ...
            seed_table_comments(session)
            self.assertEqual(len(session.scalars(select(DatabaseTableComment)).all()), 35)
