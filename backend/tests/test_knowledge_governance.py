...
def mock_model(monkeypatch, *, malformed=False, status="SUCCESS", provider_code="REAL_TEST_RESPONSE", response_mode="json", truncated=False):
    seen = []
    ...
    def chat(self, **kwargs):
        ...
        log = ModelCallLog(task_type="knowledge_graph", provider_code=provider_code, instance_code="TEST_RESPONSE",
            ...
            response_json={"choices": [{"finish_reason": "length"}]} if truncated else {},
...
def test_low_cache_size_atomic_domains_use_session_connection_for_schema_reads(db, monkeypatch):
    ...
    from app.services import resource_hub
