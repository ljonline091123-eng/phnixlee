...
public class VendorContactServiceImpl extends ServiceImpl<VendorContactMapper,VendorContact> implements IVendorContactService {
    ...
    private void submitProcess(Long id,String operateComment) {
        ...
        paramMap.put("businessTitle", "新增联系人审批");
        ...
        if (listCataLogDTOS != null) {
            ...
            if (cataLogDTOTwo != null) {
                /* 赋值使用二级单位 */
                customProcessKey = ProcessKeyEnum.ZHAOCAI_VENDOR_ADDCONTACT.getIdentifying().replace("{org}",org);
            }
            if (orgThree != null) {
                /* 判断三级单位流程是否存在 */
                ...
                if (cataLogDTOThree != null) {
                    /* 赋值使用三级单位 */
                    customProcessKey = ProcessKeyEnum.ZHAOCAI_VENDOR_ADDCONTACT.getIdentifying().replace("{org}",orgThree);
                }
            }
        }
