...
import com.zhaocai.business.integration.procurement.ContractPlanningPushEventHandler;
import com.zhaocai.business.integration.procurement.ContractPlanningPushPayload;
...
import com.zhaocai.business.procurement.dto.imports.MaterialsListImportDTO;
...
public class ProcurementPlanServiceImpl extends ServiceImpl<ProcurementPlanMapper, ProcurementPlan> implements IProcurementPlanService {
    ...
    @Autowired
    private IIntegrationOutboxService integrationOutboxService;
...
//        checkMaterialsList(requestVO);
    ...
    public void pushProcurementPlan(ProcurementPlanPushVO planPushVO) {
        String aggregateId = String.valueOf(planPushVO.getContractPlanningId());
        String idempotencyKey = String.join(":", "CONTRACT_PLANNING_PUSH", aggregateId,
                String.valueOf(planPushVO.getSchemeId()), String.valueOf(planPushVO.getNoticeId()),
                String.valueOf(planPushVO.getSplitContractId()));
        ContractPlanningPushPayload payload = new ContractPlanningPushPayload();
        payload.setPlan(planPushVO);
        payload.setSenderName(SecurityUtils.getLoginUserNickName());
        String thirdUserId = SecurityUtils.getThridUserId();
        payload.setSenderThirdUserId(StringUtils.isNotEmpty(thirdUserId) ? Long.parseLong(thirdUserId) : null);
        payload.setAuthorization(SecurityUtils.getMasterControlToken());
        IntegrationOutboxEvent event = integrationOutboxService.enqueue(
                "ContractPlanning", aggregateId, ContractPlanningPushEventHandler.EVENT_TYPE,
                idempotencyKey, JSONObject.toJSONString(payload));
        if (Integer.valueOf(IntegrationOutboxEvent.PUBLISHED).equals(event.getStatus())) {
            return;
        }
        ContractPlanningPushRecord record = savaContractPlanningPushRecord(planPushVO);
        try {
            boolean delivered = integrationOutboxService.execute(event, () -> deliverProcurementPlanPush(payload));
            if (!delivered) {
                throw new BusinessException("推送任务已被其他执行器领取，请稍后查询结果");
            }
        } catch (RuntimeException ex) {
            record.setPushStatus(ContractPlanningPushRecord.FAILED);
            contractPlanningPushRecordService.updateById(record);
            throw ex;
        }
    }

    @Override
    public void deliverProcurementPlanPush(ContractPlanningPushPayload payload) {
        dealOpenPeopleTodoTask(payload.getPlan(), payload.getSenderName(),
                payload.getSenderThirdUserId(), payload.getAuthorization());
        updatePushRecordStatus(payload.getPlan(), ContractPlanningPushRecord.SUCCEEDED);
    }
    ...
    public ContractPlanningPushRecord savaContractPlanningPushRecord(ProcurementPlanPushVO planPushVO) {
        ...
        record.setPushStatus(ContractPlanningPushRecord.PENDING);
    ...
    public void dealOpenPeopleTodoTask(ProcurementPlanPushVO planPushVO) {
        String thirdUserId = SecurityUtils.getThridUserId();
        dealOpenPeopleTodoTask(planPushVO, SecurityUtils.getLoginUserNickName(),
                StringUtils.isNotEmpty(thirdUserId) ? Long.parseLong(thirdUserId) : null,
                SecurityUtils.getMasterControlToken());
    }

    private void dealOpenPeopleTodoTask(ProcurementPlanPushVO planPushVO, String senderName,
                                        Long senderThirdUserId, String authorization) {
...
//            requestDTO.setContent(String.format(ApproveFlowPromptTemplateEnum.PROCUREMENT_PLAN_PUSH.getDesc(), planPushVO.getContractPlanningName()));
            ...
            String content =
                    /* 推送人 登录人 */
                    "发送人: " + (senderName == null ? "系统" : senderName) +
                            /* 项目名称 */
            ...
            requestDTO.setMsgFromPerCode(senderThirdUserId);
            requestDTO.setMsgFromPerName(senderName == null ? "系统" : senderName);
...
//            requestDTO.setUserObj("");
        ...
        parentRequestDTO.setAuthorization(authorization);
//        parentRequestDTO.setAuthorization("Bearer eyJhbGciOiJIUzUxMiJ9.eyJsb2dpbl91c2VyX2tleSI6IjA3YzRiZGQyLTczMTUtNGNkMC04YTAxLTEwMWNhODAxYjdhNl8xODE2MzY1MTczMyJ9.W6eLb5HmdglRIO2sle2O1sEm2ns4cJT_VNNTi-kMK9VpQIZZqs6gHF0n329G6EdCxkWjqXMGmzaYlLLxAQUbkw");
        thridPartyTodoTaskService.deliverTodoTask(parentRequestDTO);
    }

    private void updatePushRecordStatus(ProcurementPlanPushVO planPushVO, int status) {
        LambdaUpdateWrapper<ContractPlanningPushRecord> update = new LambdaUpdateWrapper<ContractPlanningPushRecord>()
                .eq(ContractPlanningPushRecord::getContractPlanningId, planPushVO.getContractPlanningId())
                .set(ContractPlanningPushRecord::getPushStatus, status);
        if (planPushVO.getSchemeId() == null) {
            update.isNull(ContractPlanningPushRecord::getSchemeId);
        } else {
            update.eq(ContractPlanningPushRecord::getSchemeId, planPushVO.getSchemeId());
        }
        if (planPushVO.getNoticeId() == null) {
            update.isNull(ContractPlanningPushRecord::getNoticeId);
        } else {
            update.eq(ContractPlanningPushRecord::getNoticeId, planPushVO.getNoticeId());
        }
        if (planPushVO.getSplitContractId() == null) {
            update.isNull(ContractPlanningPushRecord::getSplitContractId);
        } else {
            update.eq(ContractPlanningPushRecord::getSplitContractId, planPushVO.getSplitContractId());
        }
        contractPlanningPushRecordService.update(update);
    }
...
//        paramMap.put("contractMoney", procurementScheme.getCeilingPrice());/* 采购方案上限价 价格 */
    ...
    @Override
    public List<MaterialsVO> importDataTwo(List<MaterialsListImportDTO> userList, String radioType, String operName) {
        ...
        for (MaterialsListImportDTO data : userList) {
            errIndex++;
