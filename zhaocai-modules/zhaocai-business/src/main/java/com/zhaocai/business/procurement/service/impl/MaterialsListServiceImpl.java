...
public class MaterialsListServiceImpl extends ServiceImpl<MaterialsListMapper, MaterialsList> implements IMaterialsListService {
    ...
    public List<ContractSplitMaterialsVO> listMaterialsByPlanId(Long planId, Boolean isFilter) {
        ...
        Map<String, List<MaterialsList>> map = materialsList.stream()
                .collect(Collectors.groupingBy(x -> x.getSplitId() + "-_#_-" + ((x.getSplitContractName() == null || x.getSplitContractName().equals("")) ? "&" : x.getSplitContractName())
        ...
        List<DictListVO> dictListVOS = sysDictDataService.listDictByType(DictBizEnum.UNDERLING_RENT_MODE.getName());
        Map<String, String> rentModeMap = new HashMap<>();
        if (dictListVOS != null && !dictListVOS.isEmpty()) {
            rentModeMap = dictListVOS.stream().collect(Collectors.toMap(DictListVO::getDictValue, DictListVO::getDictLabel));
        }
