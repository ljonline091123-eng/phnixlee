...
public class MainActivity extends Activity {
    ...
    static final class GameView extends View {
        ...
        private static final float TOP_HEIGHT_DP = 238f;
        ...
        private static final float BOARD_MARGIN_DP = 16f;
    ...
    private boolean clearedThisTurn;
        ...
        private void drawBackground(Canvas canvas) {
            ...
            canvas.drawRoundRect(
                    boardLeft - px(7f),
                    boardTop - px(7f),
                    boardLeft + boardSize + px(7f),
                    boardTop + boardSize + px(7f),
                    px(10f),
                    px(10f),
                    paint
            );
        ...
        private void drawHeader(Canvas canvas) {
            ...
            canvas.drawRect(0f, 0f, getWidth(), topHeight(), paint);

            paint.setColor(Color.WHITE);
            paint.setFakeBoldText(true);
            paint.setTextSize(px(27f));
            canvas.drawText(dailyChallenge ? "\u6bcf\u65e5\u6311\u6218" : "\u4e94\u5b50\u6d88\u9664", px(18f), px(34f), paint);

            paint.setFakeBoldText(false);
            paint.setTextSize(px(14f));
            String subtitle = dailyChallenge
                    ? (dailyPuzzle == null ? "每日残局" : dailyPuzzle.title + " · " + dailyPuzzle.target)
                    : "白色万能球 · 炸药+四颗同色球即可清除";
            canvas.drawText(subtitle, px(18f), px(56f), paint);

            menuBounds.set(getWidth() - px(114f), px(14f), getWidth() - px(16f), px(58f));
            paint.setColor(0xffef5350);
            canvas.drawRoundRect(menuBounds, px(12f), px(12f), paint);
            paint.setColor(Color.WHITE);
            paint.setFakeBoldText(true);
            paint.setTextSize(px(16f));
            canvas.drawText("菜单", menuBounds.left + px(28f), menuBounds.centerY() + px(6f), paint);
            paint.setFakeBoldText(false);
        ...
        private void drawProfileBar(Canvas canvas) {
            ...
            profileBounds.set(px(18f), px(148f), getWidth() - px(18f), px(214f));
            paint.setStyle(Paint.Style.FILL);
            paint.setColor(0xff394b59);
            canvas.drawRoundRect(profileBounds, px(10f), px(10f), paint);
            paint.setColor(medal.color);
            canvas.drawCircle(profileBounds.left + px(25f), profileBounds.centerY(), px(17f), paint);
            paint.setColor(0xff101820);
            paint.setTextSize(medal.mark.length() > 2 ? px(8f) : px(13f));
            paint.setFakeBoldText(true);
            canvas.drawText(medal.mark, profileBounds.left + (medal.mark.length() > 2 ? px(19f) : px(20f)),
                    profileBounds.centerY() + px(5f), paint);
            paint.setColor(Color.WHITE);
            paint.setTextSize(px(18f));
            canvas.drawText(profileName + "  ·  Lv." + (medalIndex(medal) + 1) + "  " + medal.name,
                    profileBounds.left + px(52f), profileBounds.top + px(27f), paint);
            paint.setFakeBoldText(false);
            paint.setColor(0xffc9d6df);
            paint.setTextSize(px(13f));
            canvas.drawText("成长分 " + growthPoints() + "   ·   点击查看能力卡", profileBounds.left + px(52f), profileBounds.top + px(50f), paint);
            paint.setFakeBoldText(false);
            if (dailyChallenge && dailyPuzzle != null) {
                paint.setColor(0xfff3b34c);
                canvas.drawRoundRect(px(18f), px(218f), getWidth() - px(18f), px(234f), px(8f), px(8f), paint);
                paint.setColor(0xff101820);
                paint.setTextSize(px(11f));
                paint.setFakeBoldText(true);
                canvas.drawText("今日目标：" + dailyPuzzle.target, px(28f), px(230f), paint);
                paint.setFakeBoldText(false);
            }
        }
        ...
        private void drawScoreBar(Canvas canvas) {
            float left = px(18f);
            float top = px(68f);
            float right = Math.min(getWidth() * 0.43f, px(188f));
            float bottom = px(136f);
            paint.setStyle(Paint.Style.FILL);
            paint.setColor(0xff263746);
            canvas.drawRoundRect(left, top, right, bottom, px(12f), px(12f), paint);
            ...
            paint.setFakeBoldText(true);
            paint.setTextSize(px(14f));
            canvas.drawText("得分", left + px(14f), top + px(22f), paint);
            String scoreText = String.format(Locale.US, "%05d", Math.min(99999, score));
            paint.setTypeface(Typeface.create(Typeface.MONOSPACE, Typeface.BOLD));
            paint.setTextSize(px(27f));
            paint.setColor(Color.WHITE);
            canvas.drawText(scoreText, left + px(58f), bottom - px(15f), paint);
            paint.setTypeface(Typeface.DEFAULT);
            paint.setFakeBoldText(false);
        }

        private void drawPreview(Canvas canvas) {
            float left = Math.max(px(202f), getWidth() * 0.47f);
            float top = px(68f);
            float right = getWidth() - px(18f);
            float bottom = px(136f);
            paint.setStyle(Paint.Style.FILL);
            paint.setColor(0xff263746);
            canvas.drawRoundRect(left, top, right, bottom, px(12f), px(12f), paint);
            ...
            paint.setFakeBoldText(true);
            paint.setTextSize(px(14f));
            canvas.drawText("下一轮", left + px(12f), top + px(22f), paint);
            paint.setFakeBoldText(false);
            float centerY = top + px(48f);
            float firstX = left + px(82f);
            float gap = px(48f);
            for (int i = 0; i < PREVIEW_COUNT; i++) {
                if (preview[i] != EMPTY) {
                    drawPiece(canvas, preview[i], firstX + i * gap, centerY, px(16f), false);
                }
            }
        }
        ...
        private void drawFooter(Canvas canvas) {
            float top = boardTop + boardSize + px(14f);
            float bottom = Math.min(getHeight() - navigationInset - px(8f), top + px(54f));
            if (bottom <= top) return;
            paint.setStyle(Paint.Style.FILL);
            paint.setColor(0xff263746);
            canvas.drawRoundRect(px(18f), top, getWidth() - px(18f), bottom, px(12f), px(12f), paint);
            paint.setColor(dailyChallenge ? 0xffffd166 : 0xffc9d6df);
            paint.setFakeBoldText(true);
            paint.setTextSize(px(15f));
            String title = dailyChallenge ? "心形残局 · 每日仅一次" : "移动规则";
            canvas.drawText(title, px(30f), top + px(22f), paint);
            paint.setColor(Color.WHITE);
            paint.setFakeBoldText(false);
            paint.setTextSize(px(13f));
            String detail = dailyChallenge
                    ? "红色五连完成后点亮每日来客能力卡"
                    : "点击棋子，再点击可达空格；五连即可消除";
            canvas.drawText(detail, px(30f), top + px(42f), paint);
        }
        ...
        private void drawAchievementToast(Canvas canvas) {
            ...
            float top = topHeight() + px(12f);
            float bottom = top + 48f;
            canvas.drawRoundRect(left, top, right, bottom, px(20f), px(20f), paint);
            paint.setColor(Color.WHITE);
            paint.setTextSize(px(15f));
            paint.setFakeBoldText(true);
        ...
        private void beginResolution(final int token) {
            ...
            if (dailyChallenge && moveCount >= 1 && !clearedThisTurn && !dailyChallengeCompleted) {
                failDailyChallenge();
                return;
            }
        if (isFull()) {
        ...
        private void showCollectionAndChallenges() {
            if (showVisualCardsEnabled()) {
                showAbilityCardsVisual();
                return;
            }
            SharedPreferences prefs = context.getSharedPreferences(PROGRESSION_PREFS, Context.MODE_PRIVATE);
        ...
        private boolean showVisualCardsEnabled() {
            return true;
        }

        private void showAbilityCardsVisual() {
            SharedPreferences prefs = context.getSharedPreferences(PROGRESSION_PREFS, Context.MODE_PRIVATE);
            Set<String> unlocked = new HashSet<>(prefs.getStringSet("cards", Collections.emptySet()));
            LinearLayout root = new LinearLayout(context);
            root.setOrientation(LinearLayout.VERTICAL);
            ...
            ProgressionCatalog.Medal current = currentMedal();
            profile.setText(profileName + "  ·  Lv." + (medalIndex(current) + 1) + "  " + current.mark + " " + current.name
                    + "\n已点亮能力卡 " + unlocked.size() + "/54  ·  成长分 " + growthPoints());
            profile.setTextColor(Color.WHITE);
            profile.setTextSize(17f);
            profile.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
            profile.setPadding(dp(14), dp(12), dp(14), dp(12));
            profile.setBackground(cardBackground(0xff263746, 0xff607d8b));
            root.addView(profile, new LinearLayout.LayoutParams(-1, -2));

            TextView daily = new TextView(context);
            daily.setText("每日残局 MVP  ·  像素心形\n每天选择一次；完成后可继续抽取未完成图形，失败当天锁定。\n目标：移动红棋补齐心形中线五连");
            daily.setTextColor(0xff101820);
            daily.setTextSize(14f);
            daily.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
            daily.setPadding(dp(14), dp(10), dp(14), dp(10));
            daily.setBackground(cardBackground(0xffffd166, 0xfff59e0b));
            LinearLayout.LayoutParams dailyParams = new LinearLayout.LayoutParams(-1, -2);
            dailyParams.topMargin = dp(10);
            root.addView(daily, dailyParams);

            TextView medalsTitle = new TextView(context);
            medalsTitle.setText("12 阶奖牌设计  ·  当前 " + current.mark + " " + current.name);
            medalsTitle.setTextColor(0xff101820);
            medalsTitle.setTextSize(16f);
            medalsTitle.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
            medalsTitle.setPadding(0, dp(14), 0, dp(7));
            root.addView(medalsTitle, new LinearLayout.LayoutParams(-1, -2));

            HorizontalScrollView medalScroll = new HorizontalScrollView(context);
            medalScroll.setHorizontalScrollBarEnabled(false);
            LinearLayout medalRow = new LinearLayout(context);
            medalRow.setOrientation(LinearLayout.HORIZONTAL);
            for (int i = 0; i < ProgressionCatalog.MEDALS.length; i++) {
                ProgressionCatalog.Medal medal = ProgressionCatalog.MEDALS[i];
                boolean earned = growthPoints() >= medal.threshold;
                TextView item = new TextView(context);
                item.setText((i + 1) + "\n" + medal.mark + "\n" + medal.name + "\n" + medal.threshold + " 分");
                item.setGravity(Gravity.CENTER);
                item.setTextSize(11f);
                item.setTextColor(earned ? 0xff101820 : 0xff607d8b);
                item.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
                item.setPadding(dp(5), dp(6), dp(5), dp(6));
                item.setBackground(cardBackground(earned ? medal.color : 0xffd8dee3, earned ? 0xff101820 : 0xffb0bec5));
                LinearLayout.LayoutParams itemParams = new LinearLayout.LayoutParams(dp(82), dp(92));
                itemParams.rightMargin = dp(6);
                medalRow.addView(item, itemParams);
            }
            medalScroll.addView(medalRow);
            root.addView(medalScroll, new LinearLayout.LayoutParams(-1, dp(98)));

            TextView cardsTitle = new TextView(context);
            cardsTitle.setText("54 张能力卡  ·  完成条件后点亮");
            cardsTitle.setTextColor(0xff101820);
            cardsTitle.setTextSize(16f);
            cardsTitle.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
            cardsTitle.setPadding(0, dp(14), 0, dp(8));
            root.addView(cardsTitle, new LinearLayout.LayoutParams(-1, -2));

            GridLayout grid = new GridLayout(context);
            grid.setColumnCount(2);
            for (ProgressionCatalog.AbilityCard card : ProgressionCatalog.CARDS) {
                boolean isUnlocked = unlocked.contains(card.id);
                LinearLayout tile = new LinearLayout(context);
                tile.setOrientation(LinearLayout.VERTICAL);
                tile.setPadding(dp(10), dp(10), dp(10), dp(10));
                tile.setBackground(cardBackground(isUnlocked ? 0xffe7f6ec : 0xffeef1f3,
                        isUnlocked ? 0xff22a06b : 0xffb0bec5));
                TextView title = new TextView(context);
                title.setText((isUnlocked ? "● " : "○ ") + card.name);
                title.setTextColor(isUnlocked ? 0xff087443 : 0xff52616b);
                title.setTextSize(14f);
                title.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
                TextView group = new TextView(context);
                group.setText(card.group + "  ·  " + (card.equipment ? "装备卡" : "收藏卡"));
                group.setTextColor(0xff607d8b);
                group.setTextSize(11f);
                TextView condition = new TextView(context);
                condition.setText(isUnlocked ? "已点亮\n" + card.effect : "解锁：" + card.condition);
                condition.setTextColor(0xff455a64);
                condition.setTextSize(11f);
                condition.setMaxLines(3);
                tile.addView(title);
                tile.addView(group);
                tile.addView(condition);
