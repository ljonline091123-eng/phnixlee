...
public class FlowDefinitionController extends BaseController {
    ...
    @GetMapping(value = "/list/lastByKey")
    @ApiOperation(value = "通过流程标识key最新版本流程定义id", response = FlowProcDefDto.class)
    //@RequiresPermissions("flowable:definition:list")
    public String listLastByKey(@ApiParam(value = "流程标识", required = true) @RequestParam String procDefKey) {
        return flowDefinitionService.getByProcDefKey(procDefKey);
    }
